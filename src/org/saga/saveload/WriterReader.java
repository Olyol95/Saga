...
public class WriterReader {
	 ...
	 *             when write fails
	 */
	public static void write(Directory dir, String name, Object obj)
			throws IOException {
		...
		// File:
		File filedir = new File(dir.getDirectory()
				+ dir.getFilename().replace(NAME_SUBS, name));
		if (!filedir.exists()) {
	 ...
	 *             when read fails
	 */
	public static <T> T read(Directory dir, String name, Class<T> type)
			throws IOException {

		// Gson:
		GsonBuilder gsonBuilder = new GsonBuilder();
		gsonBuilder.registerTypeAdapter(Bundle.class,
				new SagaCustomSerializer());
		gsonBuilder.registerTypeAdapter(Building.class,
				new SagaCustomSerializer());
		gsonBuilder.registerTypeAdapter(BuildingSign.class,
				new SagaCustomSerializer());
		gsonBuilder.registerTypeAdapter(Ability.class,
				new SagaCustomSerializer());
		...
		if (objStr.length() == 0 || objStr.toString().equals("null")) {
			SagaLogger.severe(WriterReader.class, "null or empty data for "
					+ dir);
	 ...
	 *             when read fails
	 */
	public static <T> T readConfig(Directory dir, Class<T> type)
			throws IOException {
		...
		// Fix null and empty:
		if (objStr.length() == 0 || objStr.toString().equals("null")) {
			SagaLogger.severe(WriterReader.class, "null or empty config for "
					+ dir);
			objStr = new StringBuffer("{ }");
		}
	...
	public static boolean checkExists(Directory dir, String name) {
		return new File(dir.getDirectory()
				+ dir.getFilename().replace(NAME_SUBS, name)).exists();
	}
	...
	public static String[] getAllIds(Directory directory) {
		...
		if (names == null) {
			SagaLogger.severe(WriterReader.class,
					"could not retrieve faction names");
			names = new String[0];
		}
	...
	public static void delete(Directory dir, String name) {
		...
		File newFile = new File(dir.getDeletedDirectory()
				+ dir.getFilename().replace(NAME_SUBS, name));
		File oldFile = new File(dir.getDirectory()
				+ dir.getFilename().replace(NAME_SUBS, name));
		...
		// Check if exists.
		if (!oldFile.exists()) {
			SagaLogger.severe(WriterReader.class, "failed to move " + oldFile
		...
		for (int i = 1; i < 1000; i++) {
			if (newFile.exists()) {
				newFile.renameTo(new File(dir.getDeletedDirectory()
						+ dir.getFilename().replace(NAME_SUBS,
								name + " (" + i + ")")));
			} else {
				break;
			}
		...
		if (success) {
			...
			SagaLogger.info("Failed to move " + oldFile.getName() + " file to "
					+ newDir + ".");
		}
	...
	public static void unpackConfig(Directory configDir) throws IOException {
		...
		// try {
		// home = new
		// File(Saga.class.getProtectionDomain().getCodeSource().getLocation().toURI());
		// }
		// catch (URISyntaxException e) {
		// throw new IOException("Invalid URI for Saga: " +
		// Saga.class.getProtectionDomain().getCodeSource().getLocation());
		// }

		// Saga.jar:
		Directory dir = Directory.SAGA_JAR;
		File home = new File(System.getProperty("user.dir") + File.separator
				+ dir.getDirectory() + dir.getFilename());
		JarFile jar = new JarFile(home);

		// Config file:
		String entryPath = (Directory.CONFIG_DEFAULTS.getDirectory() + configDir
				.getFilename()).replace(File.separator, "/");
		...
		File efile = new File(configDir.getDirectory()
				+ configDir.getFilename());

		// Create file:
		if (!efile.getParentFile().exists()) {
			SagaLogger
					.info("Creating " + efile.getParentFile() + " directory.");
	 ...
	 */
	public static void writeString(Directory dir, String name, String str)
			throws IOException {
