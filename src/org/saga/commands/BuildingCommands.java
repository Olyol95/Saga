...
public class BuildingCommands {
	...
	// Buildings general:
	@Command(aliases = { "bset" }, usage = "<building name>", flags = "", desc = "Set a building on the chunk of land.", min = 1)
	@CommandPermissions({ "saga.user.building.set" })
	public static void set(CommandContext args, Saga plugin,
		...
		try {
			selBuilding = BuildingConfiguration.config().createBuilding(
					buildingName);
		} catch (InvalidBuildingException e) {
			SagaLogger.severe(SettlementCommands.class, sagaPlayer
					+ " tried to set a building with missing definition");
			sagaPlayer.error("definition missing for " + buildingName
					+ " building");
			return;
		}
		if (selBuilding == null) {
		...
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.SET_BUILDING)) {
		...
		if (selSettlement == null
				|| !selSettlement.isBuildPointsAvailable(selBuilding)) {
			sagaPlayer.message(SettlementMessages
					.notEnoughBuildingPoints(selBuilding));
			return;
		}
	...
	public static void remove(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		// Permission:
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.REMOVE_BUILDING)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}
	...
	@Command(aliases = { "bstats" }, usage = "", flags = "", desc = "Show building stats.", min = 0, max = 0)
	...
	public static void addStorage(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.STORAGE_AREA_ADD)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}
		...
		if (selBuilding.getRemainingStorageAreas() < 1) {
			sagaPlayer.message(BuildingMessages
					.storeAreasUnavailable(selBuilding));
			return;
		}
	...
	public static void removeStorage(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		Bundle selBundle = selBuilding.getChunkBundle();
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.STORAGE_AREA_REMOVE)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}

		// Retrieve storage:
		StorageArea storageArea = selBuilding.getStorageArea(sagaPlayer
				.getLocation());
	...
	@Command(aliases = { "bborder", "bflash" }, usage = "", flags = "", desc = "Show storage area border outline.", min = 0, max = 0)
	@CommandPermissions({ "saga.user.building.storage.showborder" })
	public static void storageBorder(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		Bundle selBundle = selBuilding.getChunkBundle();
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.STORAGE_AREA_FLASH)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}
		...
		for (StorageArea storageArea : storages) {
			SettlementEffectHandler.playStoreAreaFashBorder(sagaPlayer,
					storageArea);
		}
	...
	// Arena:
	@Command(aliases = { "bpvptop" }, usage = "[to_display]", flags = "", desc = "Show pvp arena top players.", min = 0, max = 1)
	@CommandPermissions({ "saga.user.building.arena.top" })
	public static void pvpTop(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		try {
			selBuilding = Building.retrieveBuilding(args, plugin, sagaPlayer,
					Arena.class);
		} catch (Throwable e) {
	...
	// Crumble arena:
	@Command(aliases = { "bsetarenay", "bsetheight", "bsety" }, usage = "", flags = "", desc = "Set arena height.", min = 0, max = 0)
	@CommandPermissions({ "saga.user.building.crumblearena.sety" })
	public static void setHeight(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		Bundle selBundle = selBuilding.getChunkBundle();
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.CRUMBLE_ARENA_SETUP)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}
	...
	@Command(aliases = { "bsetkickloc", "bsetkick" }, usage = "", flags = "", desc = "Set crumble arena kick location.", min = 0, max = 0)
	@CommandPermissions({ "saga.user.building.crumblearena.setkick" })
	public static void setKickLocation(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		Block target = sagaPlayer.getPlayer().getTargetBlock((Set<Material>)null, 16);
		...
		// Location on chunk:
		if (BundleManager.manager().getSagaChunk(target.getChunk()) == selBuilding
				.getSagaChunk()) {
			sagaPlayer.message(BuildingMessages
					.crumbleKickMustBeOutside(selBuilding));
			return;
		}

		// Set kick location:
		selBuilding.setKickLocation(new SagaLocation(target.getLocation().add(
				0.5, 0, 0.5)));

		// Inform:
		sagaPlayer
				.message(BuildingMessages.crumbleKickLocationSet(selBuilding));

	}

	// Arena:
	@Command(aliases = { "bcrumbletop" }, usage = "[to_display]", flags = "", desc = "Show crumble arena top players.", min = 0, max = 1)
	@CommandPermissions({ "saga.user.building.crumblearena.top" })
	public static void top(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void addResident(CommandContext args, Saga plugin,
		...
		if (!bundle
				.hasPermission(sagaPlayer, SettlementPermission.ADD_RESIDENT)) {
	...
	public static void removeResident(CommandContext args, Saga plugin,
		...
		// Permission:
		Bundle bundle = selBuilding.getChunkBundle();
		if (!bundle.hasPermission(sagaPlayer,
				SettlementPermission.REMOVE_RESIDENT)) {
			sagaPlayer.message(GeneralMessages.noPermission(bundle));
			return;
		}
	...
	// Town square:
	@Command(aliases = { "sspawn" }, usage = "[settlement_name]", flags = "", desc = "Spawn in a town square.", min = 0, max = 1)
	...
	@Command(aliases = { "bsetspawn" }, usage = "", flags = "", desc = "Set spawn area for town square.", min = 0, max = 0)
	@CommandPermissions({ "saga.user.building.townsquare.setspawn" })
	public static void setSpawn(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		Bundle selBundle = selBuilding.getChunkBundle();
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.TOWN_SQUARE_SET_SPAWN)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}
	...
	public static void handleSpawn(SagaPlayer sagaPlayer, Bundle bundle) {
		...
		if (SiegeManager.manager().isSieged(bundle.getId())) {
			...
			Faction attackingFaction = SiegeManager.manager()
					.getAttackingFaction(bundle.getId());

			if (playerFaction != null) {

				if (playerFaction == defendingFaction) {
					sagaPlayer.message(WarMessages
							.siegeSpawnDeny(defendingFaction));
					return;
				}

				else if (playerFaction == attackingFaction) {
					sagaPlayer.message(WarMessages
							.siegeSpawnDeny(attackingFaction));
					return;
				}
		...
		ArrayList<TownSquare> selBuildings = bundle
