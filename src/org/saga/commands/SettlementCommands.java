...
public class SettlementCommands {
	...
	public static void create(CommandContext args, Saga plugin,
		...
		if (location == null) {
			SagaLogger.severe(SettlementCommands.class,
					"saga player location is null for " + sagaPlayer.getName());
			sagaPlayer.error("failed to retrieve " + sagaPlayer
					+ " player location");
			return;
		}
		...
		// Adjacent:
		ArrayList<SagaChunk> adjacent = SagaChunk.getAllAdjacent(location
				.getChunk());
	...
	public static void claim(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		if (args.argsLength() == 1) {
			...
			if (selBundle == null) {
				sagaPlayer
						.message(GeneralMessages.invalidSettlement(groupName));
				return;
			}
		...
		if (sagaPlayer.getBundle() == selBundle) {
			...
			sagaPlayer
					.message(SettlementMessages.claimed(sagaChunk, selBundle));
		}
	...
	public static void abandon(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void resign(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	@Command(aliases = { "sdissolve" }, usage = "[settlement_name]", flags = "f", desc = "Dissolve the settlement. The -f flag (admin mode only) is required to delete large and no-delete option settlements.", min = 0, max = 1)
	@CommandPermissions({ "saga.user.settlement.delete" })
	public static void disolve(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	@Command(aliases = { "sborder", "cborder", "sflash" }, usage = "[repeat times]", flags = "", desc = "Show the outline of the chunk. Repeat and wilderness only in admin mode.", min = 0, max = 1)
	@CommandPermissions({ "saga.user.settlement.showchunkborder" })
	public static void border(CommandContext args, Saga plugin,
			final SagaPlayer sagaPlayer) {
		...
		if (sagaChunk != null) {
			...
			if (!selBundle.hasPermission(sagaPlayer,
					SettlementPermission.FLASH_CHUNK)) {
				sagaPlayer.message(GeneralMessages.noPermission(selBundle));
				return;
			}
		}
	...
	public static void invite(CommandContext args, Saga plugin,
		...
		if (args.argsLength() == 2) {
			...
			try {
				...
				sagaPlayer.message(GeneralMessages.invalidPlayer(args
						.getString(1)));
				return;
			}
		...
		if (playerBundle != null) {
			sagaPlayer.message(SettlementMessages
					.alreadyInSettlement(selPlayer));
			return;
		}

		// Already invited:
		if (selPlayer.hasBundleInvite(selBundle.getId())) {
			sagaPlayer.message(SettlementMessages.alreadyInvited(selPlayer,
					selBundle));
			return;
		}
	...
	public static void accept(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		// No invites:
		ArrayList<Bundle> inviteBundles = BundleManager.manager().getBundles(
				sagaPlayer.getBundleInvites());
		...
		sagaPlayer
				.message(SettlementMessages.haveJoined(sagaPlayer, selBundle));
	...
	public static void decline(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void quit(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void kick(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		// Kicked yourself:
		if (selPlayer == sagaPlayer
				&& sagaPlayer.getBundleId().equals(selBundle.getId())) {
			sagaPlayer.message(SettlementMessages.cantKickYourself(sagaPlayer,
					selBundle));
			return;
		}
	...
	public static void setRole(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		// Permission:
		if (!selSettlement.hasPermission(sagaPlayer,
				SettlementPermission.SET_ROLE)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}
		...
		Proficiency role;
		try {
			role = ProficiencyConfiguration.config()
					.createProficiency(roleName);
		...
		// Inform:
		selBundle.information(SettlementMessages.newRole(selPlayer,
				selSettlement, roleName));
	...
	public static void list(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void rename(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void map(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static boolean validateName(String name) {
		...
		if (ChatUtil.getComparisonString(name).length() < SettlementConfiguration
				.config().getMinNameLength()) {
			return false;
		}
		...
		for (char c : name.toCharArray()) {
			...
			if (!org.saga.utility.chat.ChatUtil.SUBSTANCE_CHARS.contains(String
					.valueOf(c))) {
				return false;
			}
