...
public class EconomyCommands {
	...
	public static void pay(CommandContext args, Saga plugin,
		...
		// Other world:
		if (!playerLocation.getWorld().getName()
				.equals(targetLocation.getWorld().getName())) {
			sagaPlayer.message(EconomyMessages.tooFarPay());
			return;
		}
	...
	public static void settlementAddCoins(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		// Permission:
		if (!selBundle
				.hasPermission(sagaPlayer, SettlementPermission.ADD_COINS)) {
	...
	public static void settlementRemoveCoins(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.REMOVE_COINS)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}
	...
	public static void settlementBuyClaims(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		// Permission:
		if (!selBundle.hasPermission(sagaPlayer,
				SettlementPermission.BUY_CLAIMS)) {
			sagaPlayer.message(GeneralMessages.noPermission(selBundle));
			return;
		}
		...
		for (int i = 0; i < amount; i++) {
			...
			if (!selSettlement.checkRequirements()) {
				...
				if (i == 0) {
					sagaPlayer.message(GeneralMessages
							.requirementsNotMet(selSettlement));
					return;
				} else {
					break;
				}
			...
			// Check cost:
			Double cost = EconomyConfiguration.config().getClaimPointCost(
					selSettlement.getTotalClaims());
		...
		if (increased == 0) {
			selBundle.information(EconomyMessages.insufficient(selBundle),
					sagaPlayer);
			return;
		}

		// Inform:
		sagaPlayer.message(EconomyMessages.settlementBoughtClaims(increased,
				spent));

	}

	@Command(aliases = { "sbuybuildpoints", "buybpoints" }, usage = "[settlement_name] [amount]", flags = "", desc = "Buy additional build points for the settlement.", min = 0, max = 2)
	@CommandPermissions({ "saga.user.economy.settlements.buybuildpoints" })
	public static void settlementBuyBuildPoints(CommandContext args,
		...
		for (int i = 0; i < amount; i++) {
			...
			// Check cost:
			Double cost = EconomyConfiguration.config().getBuildPointCost(
					selSettlement.getTotalClaims());
			Double coins = selSettlement.getCoins();
			if (cost > coins)
				break;
		...
		// Inform:
		sagaPlayer.message(EconomyMessages.settlementBoughtBuildPoints(
				increased, spent));
	...
	public static void factionAddCoins(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void factionRemoveCoins(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		if (!selFaction.hasPermission(sagaPlayer,
				FactionPermission.REMOVE_COINS)) {
			sagaPlayer.message(GeneralMessages.noPermission(selFaction));
			return;
		}
