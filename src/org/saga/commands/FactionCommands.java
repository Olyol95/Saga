...
public class FactionCommands {
	...
	public static void accept(CommandContext args, Saga plugin,
		...
		// No invites:
		ArrayList<Faction> inviteFactions = FactionManager.manager()
				.getFactions(sagaPlayer.getFactionInvites());
	...
	public static void kick(CommandContext args, Saga plugin,
		...
		if (selFaction.isLimitedMember(selPlayer)) {
			sagaPlayer.message(WarMessages
					.limitedMembersCantBeKicked(selFaction));
			return;
		}
		...
		if (selPlayer.equals(sagaPlayer)) {
			sagaPlayer.message(FactionMessages.cantKickYourself(sagaPlayer,
					selFaction));
			return;
		}
		...
		// Inform:
		selFaction.information(FactionMessages.playerKicked(selPlayer,
				selFaction));
	...
	public static void resign(CommandContext args, Saga plugin,
		...
		case 2:
			...
			if (!selFaction.isMember(targetName)) {
				sagaPlayer.message(FactionMessages.notMember(selFaction,
						targetName));
				return;
			}
		...
		// Inform:
		selFaction
				.information(FactionMessages.newOwner(selFaction, targetName));
	...
	public static void setRank(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		if (args.argsLength() == 3) {
			...
			targetName = selFaction.matchName(args.getString(0));
			rankName = GeneralMessages.nameFromArg(args.getString(1))
					.toLowerCase();
		...
		if (selFaction.isLimitedMember(selPlayer)) {
			sagaPlayer.message(WarMessages
					.limitedMembersCantHaveRanks(selFaction));
			return;
		}
		...
		Proficiency rank;
		try {
			rank = ProficiencyConfiguration.config()
					.createProficiency(rankName);
		} catch (InvalidProficiencyException e) {
		...
		// Rank available:
		if (!selFaction.isRankAvailable(rank.getName())) {
			sagaPlayer.message(FactionMessages.rankUnavailable(selFaction,
					rankName));
			return;
		}
		...
		// Inform:
		selFaction.information(FactionMessages.newRank(selFaction, rankName,
				selPlayer));
	...
	public static void disband(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void chat(CommandContext args, Saga plugin,
		...
		// Inform allies:
		Collection<Faction> allyFactions = WarManager.manager()
				.getAllyDeclarations(selFaction.getId());
	...
	public static void setColour1(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
		...
		if (color == null || color == ChatColor.MAGIC) {
			...
			sagaPlayer.message(FactionMessages.possibleColors(colors,
					selFaction));
			return;
		}
	...
	public static void setColour2(CommandContext args, Saga plugin,
			SagaPlayer sagaPlayer) {
	...
	public static void rename(CommandContext args, Saga plugin,
		...
		// Check name:
		if (!newName.equals(selFaction.getName())
				&& FactionManager.manager().matchFaction(newName) != null) {
		...
		if (selFaction.isFormed() && cost > 0
				&& EconomyConfiguration.config().isEnabled()) {
	...
	public static boolean validateName(String str) {
		...
		if (org.saga.utility.chat.ChatUtil.getComparisonString(str).length() < FactionConfiguration
