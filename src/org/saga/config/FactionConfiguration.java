...
public class FactionConfiguration {
	...
	public void complete() {
		...
		if (siegePtsPerSecondAffiliatedModifier == null) {
			SagaLogger
					.nullField(getClass(), "siegePtsPerSecondAffiliatedBonus");
		...
		if (siegeInProgressRemindIntervalMinutes == null) {
			SagaLogger.nullField(getClass(),
					"siegeInProgressRemindIntervalMinutes");
	...
	/**
	 * Gets the members required from both factions to be able to claim
	 * settlements.
	 * 
	...
	/**
	 * Gets the minutes after which a new war can be declared after declaring
	 * peace.
	 * 
	...
	/**
	 * Gets the interval after which the factions are reminded of the siege when
	 * it is in progress.
	 * 
	...
	public static FactionConfiguration load() {
		...
		if (!WriterReader.checkExists(Directory.FACTION_CONFIG)) {
			...
			try {
				...
				SagaLogger.severe(FactionConfiguration.class,
		...
		try {
			...
			config = WriterReader.readConfig(Directory.FACTION_CONFIG,
					FactionConfiguration.class);
