...
public class SettlementConfiguration {
	 ...
	 */
	private TwoPointFunction buildPoints; // TODO Remove unused build points per
											// claims function.
	...
	public boolean checkBuildingRequirements(Settlement settlement) {
		...
		for (Entry<String, Integer> requirement : requirments) {
			...
			if (totalClaims >= bldgClaims
					&& settlement.getFirstBuilding(bldgName) == null)
				return false;
	...
	public Integer getHierarchyMin() {
		...
		for (Integer hierarchy : roleHierarchy) {
			if (hierarchy < minHierarchy || minHierarchy == -1)
				minHierarchy = hierarchy;
		}
	...
	public static SettlementConfiguration load() {
		...
		try {
			...
			config = WriterReader.readConfig(Directory.SETTLEMENT_CONFIG,
					SettlementConfiguration.class);
