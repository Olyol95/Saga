...
public class EconomyConfiguration {
	 ...
	 * 
	 * @param claims
	 *            total settlement claims available
	 ...
	 * 
	 * @param bpoints
	 *            total building points available
	...
	public static Integer nextGaussian(Integer value, Double spreadRelative) {
		...
		return (int) Math.round(nextGaussian(value.doubleValue(),
				spreadRelative));
	...
	public Double getSettlementMemberPercent() {
		double total = settlementMemberShare + settlementShare
				+ settlementFactionShare;
		if (total == 0.0)
			return 0.0;
	...
	/**
	 * Gets the reward for a killer player with a certain hierarchy level. 0
	 * hierarchy if none.
	 * 
	 * @param killedPlayer
	 *            , player killed
	 * @param killedFaction
	 *            faction where the killed player was from, null if none
	 * @return reward gained
	 */
	public Double getFactionKillReward(SagaPlayer killedPlayer,
			Faction killedFaction) {

		Proficiency rank = null;
		if (killedFaction != null)
			rank = killedFaction.getRank(killedPlayer.getName());

		if (rank != null)
			return factionKillReward.value(rank.getHierarchy());
	...
	public static EconomyConfiguration load() {
		...
		if (!WriterReader.checkExists(Directory.ECONOMY_CONFIG)) {
			...
			try {
				...
				SagaLogger.severe(EconomyConfiguration.class,
		...
		try {
			...
			config = WriterReader.readConfig(Directory.ECONOMY_CONFIG,
					EconomyConfiguration.class);
