...
public class ProficiencyConfiguration {
	 ...
	 * gson load. If not, it fills the field with defaults. Abilities are
	...
	public ArrayList<ProficiencyDefinition> getDefinitions(
		...
		for (ProficiencyDefinition definition : definitions) {
			...
			if (definition.getType() == type
					&& definition.getHierarchyLevel() == hierarchy)
				filteredDefs.add(definition);
	...
	public ArrayList<String> getProficiencyNames(ProficiencyType type) {
		...
		for (ProficiencyDefinition definition : definitions) {
			...
			if (definition.getType().equals(type))
				proficiencyNames.add(definition.getName());
	 ...
	 * @throws InvalidProficiencyException
	 *             when there is no proficiency associated with the given name
	 */
	public Proficiency createProficiency(String name)
			throws InvalidProficiencyException {

		ProficiencyDefinition definition = ProficiencyConfiguration.config()
				.getDefinition(name);
	...
	public static ProficiencyConfiguration load() {
		...
		if (!WriterReader.checkExists(Directory.PROFICIENCY_CONFIG)) {
			...
			try {
				...
				SagaLogger.severe(ProficiencyConfiguration.class,
		...
		try {
			...
			config = WriterReader.readConfig(Directory.PROFICIENCY_CONFIG,
					ProficiencyConfiguration.class);
