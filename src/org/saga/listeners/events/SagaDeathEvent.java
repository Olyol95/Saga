...
public class SagaDeathEvent {
	...
	public SagaDeathEvent(EntityDeathEvent event) {
		...
		if (event.getEntity().getLastDamageCause() != null)
			damEvent = new SagaDamageEvent(event.getEntity()
					.getLastDamageCause());
	...
	public void apply() {
		...
		if (damEvent.isPvC()) {
			...
			// Get exp:
			Double exp = ExperienceConfiguration.config().getExp(
					damEvent.creatureDefender);

			// Unnatural spawn:
			if (event.getEntity().hasMetadata(SpawnerTag.METADATA_KEY)) {

				// Modify enchant points:
				event.setDroppedExp(TwoPointFunction
						.randomRound(ExperienceConfiguration.config().spawnerEncPointMult
								* event.getDroppedExp()));
			...
			// Statistics:
			String creatureName = damEvent.creatureDefender.getClass()
					.getSimpleName().replace("_", " ").toLowerCase()
					.replace("craft", "");
		...
		// Killed a player:
			...
			// Award exp:
			Double exp = ExperienceConfiguration.config().getExp(
					damEvent.sagaDefender);
			damEvent.sagaAttacker.awardExp(exp);

			// Statistics:
			Integer usedAttr = damEvent.sagaDefender.getUsedAttributePoints();
			String range = "";
			for (int maxi = 0; maxi <= AttributeConfiguration.config()
					.getMaxAttributeCap(); maxi += 10) {
