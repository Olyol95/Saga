...
public class PlayerListener implements Listener {
	...
	public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
		...
		// Forward to chunk:
		if (sagaChunk != null)
			sagaChunk.onPlayerCommandPreprocess(sagaPlayer, event);

		if (event.isCancelled())
			return;

		// Handle command:
		if (Saga.plugin().handleCommand(event.getPlayer(), split,
				event.getMessage())) {
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		...
		if (sagaPlayer == null) {
			SagaLogger.severe(PlayerListener.class,
					"can't continue with onPlayerJoin, because the saga player for "
							+ event.getPlayer().getName() + " isn't loaded");
			return;
		}
	...
	public void onPlayerQuit(PlayerQuitEvent event) {
		...
		if (sagaPlayer == null) {
			SagaLogger.severe(PlayerListener.class,
					"can't continue with onPlayerQuit, because the saga player for "
							+ event.getPlayer().getName() + " isn't loaded");
			return;
		}
		...
		// Forward to bundle:
		if (sagaPlayer.getBundle() != null)
			sagaPlayer.getBundle().onMemberQuit(event, sagaPlayer);
	...
	public void onPlayerRespawn(PlayerRespawnEvent event) {
		...
		// Forward to chunk group:
		if (bundle != null)
			bundle.onMemberRespawn(sagaPlayer, event);
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		// Get Saga chunk:
		Location location = event.getClickedBlock() != null ? event
				.getClickedBlock().getLocation() : event.getPlayer()
				.getLocation();
		...
		if (SagaBuildEvent.isBuildEvent(event)) {
			...
			SagaBuildEvent bldEvent = new SagaBuildEvent(event, sagaPlayer,
					sagaChunk);
			SagaEventHandler.handleBuild(bldEvent);
			if (bldEvent.isCancelled())
				return;
	...
	public void onFoodLevelChange(FoodLevelChangeEvent event) {
		...
		// Saga disabled:
		if (GeneralConfiguration.isDisabled(event.getEntity().getWorld()))
			return;
	...
	private void handleChunkChange(SagaPlayer sagaPlayer, PlayerMoveEvent event) {
		...
		// Forward to chunk:
		if (sagaChunk2 != null)
			sagaChunk2.onPlayerEnter(sagaPlayer, sagaChunk1);
		if (sagaChunk1 != null)
			sagaChunk1.onPlayerLeave(sagaPlayer, sagaChunk2);
