...
public class Faction implements MinuteTicker, DaytimeTicker {
	...
	public boolean complete() {
		...
		if (claims == null) {
			...
			double maxLevel = 50;
			if (level != null) {
				claims = level.doubleValue() / maxLevel
						* FactionConfiguration.config().getMaxClaims();
			} else {
				claims = 0.0;
			}
			integrity = false;
		...
		while (playerNames.hasMoreElements()) {
			...
			try {
				...
				SagaLogger.severe(this, "tried to add an invalid " + rank
						+ " rank:" + e.getMessage());
				playerRanks.remove(playerName);
			} catch (NullPointerException e) {
				SagaLogger.severe(this, "tried to add a unitialized " + rank
						+ " rank");
				playerRanks.remove(playerName);
			}
	...
	public static Faction create(String factionName, SagaPlayer owner) {
		...
		// Create:
		Faction faction = new Faction(FactionManager.manager()
				.getUnusedFactoinId(), factionName);
		faction.complete();
		...
		// try {
		// Proficiency rank =
		// ProficiencyConfiguration.config().createProficiency(FactionConfiguration.config().getOwnerRank());
		// faction.setRank(owner, rank);
		// } catch (InvalidProficiencyException e) {
		// SagaLogger.severe(faction, "failed to set " +
		// FactionConfiguration.config().getOwnerRank() +
		// " rank, because the rank name is invalid");
		// }
	...
	public void addMember(SagaPlayer sagaPlayer) {
		...
		if (members.contains(sagaPlayer.getName()))
			SagaLogger.severe(this, "tried to add an already existing member "
					+ sagaPlayer.getName());
		...
		// Set default rank:
		try {
			Proficiency rank = ProficiencyConfiguration.config()
					.createProficiency(
							FactionConfiguration.config().getDefaultRank());
	...
	public final void removeMember(String playerName) {
		...
		if (!members.contains(playerName)) {
			SagaLogger.severe("Tried to remove a non-member " + playerName
					+ " player");
			return;
		}
		...
		try {
			...
			SagaLogger.severe(this, "could not remove " + playerName
					+ " player, because the player doesent exist");
			return;
		}
	...
	public Collection<SagaPlayer> getRawOnlineMembers() {
		...
		for (SagaPlayer onlinePlayer : onlinePlayers) {
			...
			if (isMember(onlinePlayer.getName()))
	...
	public Collection<SagaPlayer> getOnlineMembers() {
		...
		for (SagaPlayer sagaPlayer : onlinePlayers) {
			...
			if (sagaPlayer.getFactionId().equals(getId()))
				onlineMembers.add(sagaPlayer);
	...
	public Collection<SagaPlayer> getLimitedOnlineMembers() {
		...
		for (SagaPlayer sagaPlayer : onlinePlayers) {
			...
			if (isLimitedMember(sagaPlayer))
				onlineLimited.add(sagaPlayer);
	...
	public boolean isLimitedMember(SagaPlayer sagaPlayer) {
		...
		return !isMember(sagaPlayer.getName())
				&& sagaPlayer.getFactionId().equals(getId());
	...
	public boolean isMemberOnline(String playerName) {
		...
		return isMember(playerName)
				&& Saga.plugin().isSagaPlayerLoaded(playerName);
	...
	public void payCoins(Double amount) {
		...
		Double factionShare = amount
				* EconomyConfiguration.config().getFactionPercent();
		Double membersShare = amount
				* EconomyConfiguration.config().getFactionMemberPercent();
	...
	public HashSet<Integer> getAllyInvites() {
		...
		if (allies != null) {
			for (Integer allyID : allies) {
				SagaLogger.info(
						this,
						"migrating faction ID " + allyID
								+ " ally declaration to "
								+ WarManager.class.getSimpleName());
	...
	public boolean isRankAvailable(String rankName) {
		...
		ProficiencyDefinition rank = ProficiencyConfiguration.config()
				.getDefinition(rankName);
		if (rank == null)
			return false;

		if (rank.getHierarchyLevel() == FactionConfiguration.config()
				.getHierarchyMin())
			return true;
	...
	public ArrayList<String> getMembersForRanks(String rankName) {
		...
		for (String member : allMembers) {
			...
			if (rank != null && rank.getName().equals(rankName))
	...
	public boolean clockMinuteTick() {
		...
		if (claims < FactionConfiguration.config().getMaxClaims()) {
			...
			// Statistics:
			StatisticsManager.manager().addFactionClaims(this,
					FactionConfiguration.config().getClaimsPerMinute(online));
	...
	public void chat(SagaPlayer sagaPlayer, String message) {
		...
		message = getColour2() + "[" + getColour1()
				+ FactionMessages.rankedPlayer(this, sagaPlayer) + getColour2()
				+ "] " + message;
	...
	public void information(String message, SagaPlayer target) {
		...
		message = getColour2() + "[" + getColour1() + "info" + getColour2()
				+ "] " + message;
	...
	public static Faction load(String id) {
		...
		try {
			...
			faction = WriterReader.read(Directory.FACTION_DATA, id,
					Faction.class);
	...
	public void save() {
		...
		try {
			...
			SagaLogger.severe(this, "write failed: "
	...
	public void onPvPAttack(SagaDamageEvent event) {
		...
		if (event.sagaDefender instanceof SagaPlayer)
			defenderPlayer = (SagaPlayer) event.sagaDefender;
		...
		if (defenderPlayer != null) {
			...
			// Ally:
			if (WarManager.manager().isAlly(getId(),
					defenderPlayer.getFactionId())) {
	...
	public void onPvpKill(SagaPlayer attacker, SagaPlayer defender) {
		...
		if (!(attackerFaction != null && attackerFaction == this && defenderFaction != attackerFaction))
			return;
		...
		if (attackerFaction == this && defenderFaction != this) {
			...
			if (dailyKills.add(attacker.getName())) {
				...
				Double reward = EconomyConfiguration.config()
						.getFactionKillReward(defender, defenderFaction);
				payCoins(reward);
	...
	public enum FactionPermission {
		...
		// Management:
		DELETE, DISBAND, INVITE, KICK, QUIT, SET_RANK, RESIGN,
		...
		// Siege:
		DECLARE_SIEGE, UNCLAIM,

		// War:
		START_WAR, END_WAR,

		// Alliance:
		FORM_ALLIANCE, DECLINE_ALLIANCE, BREAK_ALLIANCE,
