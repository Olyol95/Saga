...
public class GeneralMessages {
	...
	public static String notSettlement(Bundle bundle) {
		return Colour.negative + "Chunk bundle " + bundle.getName()
				+ " isn't a settlement.";
	}
	...
	public static String highlight(String command) {
		...
		return ChatColor.UNDERLINE + command + CustomColour.NORMAL_FORMAT;

	}

	public static String command(String command) {

		return ChatColor.ITALIC + command + CustomColour.NORMAL_FORMAT;
	...
	public static String noPermission(Faction faction) {
		return Colour.negative + "You don't have permission to do that ("
				+ faction.getColour1() + faction.getName() + Colour.negative
				+ " faction).";
	}

	public static String noCommandPermission(Bundle bundle, String command) {
		return Colour.negative + "You don't have permission use " + command
				+ " command (" + bundle.getName() + " settlement).";
	}

	public static String noPermissionFlag(Character flag) {
		return Colour.negative + "You don't have permission to use the -"
				+ flag + " flag.";
	}
	...
	// Bars:
	public static String tugBar(ChatColor colLeft, ChatColor colRight,
			String middle, ChatColor colGeneral, Double progress) {
		...
		return colGeneral + "{" + colLeft + ChatUtil.repeat("|", left)
				+ colGeneral + middle + colRight + ChatUtil.repeat("|", right)
				+ colGeneral + "}";

	}

	public static String tugBarLeft(ChatColor colLeft, ChatColor colRight,
			ChatColor colGeneral, Double progress) {
		return tugBar(colLeft, colRight, "[<]", colGeneral, progress);
	}

	public static String tugBarRight(ChatColor colLeft, ChatColor colRight,
			ChatColor colGeneral, Double progress) {
		return tugBar(colLeft, colRight, "[>]", colGeneral, progress);
	}

	public static String tugBarStop(ChatColor colLeft, ChatColor colRight,
			ChatColor colGeneral, Double progress) {
		return tugBar(colLeft, colRight, "[*]", colGeneral, progress);
	}
	...
	public static String requirementsNotMet(Bundle bundle) {
		return Colour.negative + "Requirements not met (" + bundle.getName()
				+ " settlement).";
	}
