...
public class SettlementEffectHandler {
		...
		for (int x = xmin; x <= xmax; x++) {
			...
			for (int z = zmin; z <= zmax; z++) {
				...
				Location location = new Location(world, x + 0.5,
						world.getHighestBlockYAt(x, z) + 0.5, z + 0.5);

				if (x != xmin && x != xmax && z != zmin && z != zmax)
					continue;

				sagaPlayer.playGlobalEffect(Effect.MOBSPAWNER_FLAMES, 0,
						location);
	...
	public static void playBuildingSet(SagaPlayer sagaPlayer, Building building) {
		...
		for (int x = xmin + shift; x <= xmax - shift; x++) {
			...
			for (int z = zmin + shift; z <= zmax - shift; z++) {
				...
				if (x != xmin + shift && x != xmax - shift && z != zmin + shift
						&& z != zmax - shift)
					continue;
	...
	public static void playBuildingRemove(SagaPlayer sagaPlayer,
			Building building) {
	...
	public static void playBuildingUpgrade(SagaPlayer sagaPlayer,
			Building building) {
	...
	public static void playStoreAreaCreate(SagaPlayer sagaPlayer,
			StorageArea storeArea) {
	...
	public static void playStoreAreaRemove(SagaPlayer sagaPlayer,
			StorageArea storeArea) {
		playStoreAreaCreate(sagaPlayer, storeArea);
	}

	public static void playStoreAreaFashBorder(SagaPlayer sagaPlayer,
			StorageArea storeArea) {
		playStoreAreaCreate(sagaPlayer, storeArea);
	}
