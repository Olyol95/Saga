...
public class AdminMessages {
	...
	public static String playerExpSet(Integer exp, SagaPlayer sagaPlayer) {
		return Colour.positive + "Player " + sagaPlayer.getName()
				+ " exp set to " + exp + ".";
	}
	...
	public static String playerExpOutOfRange(String exp) {
		return Colour.negative + "Exp " + exp
				+ " is out of range. Allowed range: 0 - "
				+ ExperienceConfiguration.config().getMaxExp() + ".";
	}

	// Settlement and faction claims:
	public static String settleClaimsOutOfRange(String claims) {
		return Colour.negative + "Claims " + claims
				+ " is out of range. Allowed range: 0 - "
				+ SettlementConfiguration.config().getMaxClaims() + ".";
	}

	public static String settleBuildPointsOutOfRange(String bldPts) {
		return Colour.negative + "Build points " + bldPts
				+ " is out of range. Allowed range: 0 - "
				+ SettlementConfiguration.config().getMaxBuildPoints() + ".";
	}
	...
	public static String setClaims(Settlement settlement) {
		return Colour.positive + "Settlement " + settlement.getName()
				+ " claims set to " + settlement.getTotalClaims() + ".";
	}

	public static String setBuildPoints(Settlement settlement) {
		return Colour.positive + "Settlement " + settlement.getName()
				+ " build points set to "
				+ settlement.getAvailableBuildPoints() + ".";
	}
	...
	public static String attributeSet(String attribute, Integer score) {
		return Colour.positive + ChatUtil.capitalize(attribute)
				+ " was set to " + score + ".";
	}

	public static String attributeSet(String attribute, Integer score,
			SagaPlayer selPlayer) {
		return Colour.positive + "Players " + selPlayer.getName() + " "
				+ attribute + " was set to " + score + ".";
	}
	...
	public static String attributeScoreOutOfRange(String score) {
		return Colour.negative + "Attribute score " + score
				+ " is out of range. Allowed range: 0 - "
				+ AttributeConfiguration.config().getMaxAttributeCap() + ".";
	}
	...
	public static String borderRepeatAdminModeOnly() {
		return Colour.negative
				+ "Repeat function is only available in admin mode.";
	}

	public static String borderWildernessAdminModeOnly() {
		return Colour.negative
				+ "Admin mode required to use border command in the wilderness.";
	}
	...
	private static String parameterKey(AttributeParameter parameter) {
		...
		return parameter.toString().toLowerCase().replace("_", " ")
				.replace(" modifier", "").replace(" multiplier", "")
				.replace("penetration", "pen");

	}

	private static String parameterValue(AttributeParameter parameter,
			Double value) {
	...
	public static String writeDone(Directory dir, String name) {
		return Colour.positive + "Write complete: " + dir.getDirectory()
				+ dir.getFilename().replace(WriterReader.NAME_SUBS, name) + ".";
	}
	...
	public static String wikiCommands(ArrayList<Method> commandMethods) {
		...
		Comparator<Method> comparator = new Comparator<Method>() {
			...
			public int compare(Method o1, Method o2) {
				...
				CommandPermissions perm1 = o1
						.getAnnotation(CommandPermissions.class);
				CommandPermissions perm2 = o2
						.getAnnotation(CommandPermissions.class);
				if (perm1 == null || perm2 == null || perm1.value().length == 0
						|| perm2.value().length == 0)
		...
		for (String category : categories) {
			...
			for (Method method : commandMethods) {
				...
				CommandPermissions permissions = method
						.getAnnotation(CommandPermissions.class);
				String permission = "";
				if (permissions != null && permissions.value().length != 0)
					permission = permissions.value()[0];
		...
		for (int b = 0; b < books.length; b++) {
			...
			for (int i = 0; i < book.sections(); i++) {
				...
				if (sect.startsWith(ChatBook.TABLE.toString())) {
					...
					result.append(format(table, start, left, middle, right,
							next, end));
		...
		for (Attribute attribute : attributes) {
			...
			for (Entry<AttributeParameter, TwoPointFunction> parameter : parameters) {
				...
				for (int j = init; j <= max; j += step) {
					...
					result.append(parameterValue(parameter.getKey(), parameter
							.getValue().value(j)));
	...
	public static String nextDaytime(World world, Daytime daytime) {
		...
		return Colour.positive + "Daytime set to " + daytime + " for world "
				+ world.getName() + ".";
