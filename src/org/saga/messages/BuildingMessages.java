...
public class BuildingMessages {
	...
	public static String buildingCommandRestrict(String correctBuildingName,
			String command) {
		return Colour.negative + "Command " + command
				+ " can only be used from a " + correctBuildingName + ".";
	}
	...
	public static String unavailable(Building building) {
		...
		return Colour.negative + ChatUtil.capitalize(building.getDisplayName())
				+ " isn't available.";
	...
	public static String produced(ArrayList<SagaItem> items) {
		...
		if (items.size() != 0) {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (i != 0 && items.get(i - 1).getType() == item.getType())
					duplic = true;
				if (i != items.size() - 1
						&& items.get(i + 1).getType() == item.getType())
					duplic = true;
	...
	public static String storeAreaSingleChunk() {
		return Colour.negative
				+ "Storage area must be on the same chunk of land.";
	}
	...
	public static String storeAreaRemoved(Building building) {
		return Colour.positive + "Storage area removed from "
				+ building.getName() + ".";
	}
	...
	public static String storeAreasUnavailable(Building building) {
		return Colour.negative + "No storage areas available for "
				+ building.getName() + ".";
	}
	...
	public static String stored(Material material, Building building) {
		...
		if (material == Material.CHEST)
			return Colour.positive + "Added item storage.";
	...
	public static String withdrew(Material material, Building building) {
		...
		if (material == Material.CHEST)
			return Colour.positive + "Removed item storage.";

		return Colour.positive + "Withdrew "
				+ GeneralMessages.material(material) + ".";
	...
	public static String arenaTop(Arena arena, Integer count) {
		...
		// Fix count:
		if (listLen > topPlayers.size())
			listLen = topPlayers.size();
		...
		for (ArenaPlayer arenaPlayer : topPlayers) {
			...
			if (arenaPlayer.getDeaths() == 0) {
				...
				kdr = ChatUtil.displayDouble(arenaPlayer.getKills()
						.doubleValue() / arenaPlayer.getDeaths().doubleValue());
			}

			table.addLine(new String[] { arenaPlayer.getName(),
					arenaPlayer.calculateScore().intValue() + "",
					arenaPlayer.getKills().toString(),
					arenaPlayer.getDeaths().toString(), kdr });

		}

		table.collapse();

		return ChatFramer.frame("top " + count, table.createTable(),
				messageColor.nextColour());
	...
	public static String crumbleKickMustBeOutside(CrumbleArena arena) {
		return Colour.negative
				+ "Kick location can't be on the same chunk as the "
				+ arena.getName() + ".";
	}
	...
	public static String arenaTop(CrumbleArena arena, Integer count) {
		...
		// Names:
		table.addLine(new String[] { GeneralMessages.tableTitle("name"),
				GeneralMessages.tableTitle("wins"),
				GeneralMessages.tableTitle("losses"),
				GeneralMessages.tableTitle("WLR") });
		...
		for (CrumblePlayer arenaPlayer : topPlayers) {
			...
			if (arenaPlayer.getLosses() == 0) {
				...
				wlr = ChatUtil.displayDouble(arenaPlayer.getWins()
						.doubleValue() / arenaPlayer.getLosses().doubleValue());
			}

			table.addLine(new String[] { arenaPlayer.getName(),
					arenaPlayer.getWins().toString(),
					arenaPlayer.getLosses().toString(), wlr });
	...
	public static String attributeMaxReached(String attribute, Integer max) {
		return Colour.negative + ChatUtil.capitalize(attribute)
				+ " can't be trained above " + max + ".";
	}

	public static String attributePointsRequired(String attribute) {
		return Colour.negative + "Not enough attribute points to train "
				+ attribute + ".";
	}

	// Attribute reset sign:
	public static String resetAttr(String attribute, Integer score) {
		return Colour.normal1 + ChatUtil.capitalize(attribute) + " reset to "
				+ score + ".";
	}
	...
	public static String abilityTrained(Ability ability, Integer score) {
		...
		if (score == 1)
			return Colour.positive + "Learned " + ability.getName() + ".";
		return Colour.positive + "Upgraded to " + ability.getName() + " "
				+ RomanNumeral.binaryToRoman(score) + ".";

	}

	public static String abilityRequirementsNotMet(String abilityName,
			Integer score) {
		return Colour.negative + "Requirements not met for " + abilityName
				+ " " + RomanNumeral.binaryToRoman(score) + ".";
	}

	public static String abilityMaxReached(String abilName) {
		return Colour.negative + ChatUtil.capitalize(abilName)
				+ " can't be upgraded any further.";
	}

	public static String abilityPointsRequired(String ability) {
		return Colour.negative + "Not enough ability points to upgrade "
				+ ability + ".";
	}

	// Ability reset:
	public static String abilityAlreadyReset(String ability) {
		return Colour.negative + ChatUtil.capitalize(ability)
				+ " is already reset.";
	}
	...
	public static String abilityReset(Ability ability, Integer score) {
		...
		if (score <= 0)
			return Colour.positive + "Reset " + ability.getName() + ".";
		return Colour.positive + ChatUtil.capitalize(ability.getName())
				+ " reset to " + RomanNumeral.binaryToRoman(score) + ".";

	}

	// Town square:
	public static String noTownSquare(Bundle bundle) {
		return Colour.negative + "" + bundle.getName() + " doesn't have a "
				+ ChatUtil.className(TownSquare.class) + ".";
	}
