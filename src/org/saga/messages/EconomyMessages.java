...
public class EconomyMessages {
	...
	public static String settlementBoughtClaims(Integer amount, Double cost) {
		...
		if (amount == 1) {
			return Colour.positive + "Bought a claim for "
					+ EconomyMessages.coins(cost) + ".";
		} else {
			return Colour.positive + "Bought " + amount + " claims for "
					+ EconomyMessages.coins(cost) + ".";
		}
	...
	public static String settlementBoughtBuildPoints(Integer amount, Double cost) {
		...
		if (amount == 1) {
			return Colour.positive + "The settlement bought a build point for "
					+ EconomyMessages.coins(cost) + ".";
		} else {
			return Colour.positive + "The settlement bought " + amount
					+ " build points for " + EconomyMessages.coins(cost) + ".";
		}

	}

	public static String settlementAddedCoins(Double amount) {
		return Colour.positive + "Deposited " + EconomyMessages.coins(amount)
				+ " to the settlements bank.";
	}

	public static String settlementRemovedCoins(Double amount) {
		return Colour.positive + "Withdrew " + EconomyMessages.coins(amount)
	...
	public static String paid(SagaPlayer paidPlayer, Double amount) {
		return Colour.positive + "Paid " + coins(amount) + "s to "
				+ paidPlayer.getName() + ".";
	}

	public static String gotPaid(SagaPlayer payerPlayer, Double amount) {
		return Colour.positive + "Got paid " + coins(amount) + "s by "
				+ payerPlayer.getName() + ".";
	}

	public static String tooFarPay(Double maximumDistance) {
		return Colour.negative + "Need to be within "
				+ maximumDistance.intValue() + "blocks to pay.";
	}
	...
	public static String walletModified(SagaPlayer targetPlayer, Double amount) {
		...
		if (amount >= 0)
			return Colour.positive + "Added " + EconomyMessages.coins(amount)
					+ " to players " + targetPlayer.getName() + " wallet.";

		return Colour.positive + "Removed " + EconomyMessages.coins(amount)
				+ " from players " + targetPlayer.getName() + " wallet.";
	...
	public static String insufItems(Material items) {
		...
		return Colour.negative + "Not enough "
				+ GeneralMessages.material(items) + ".";
	...
	public static String sold(Material item, Integer amount, Double price) {
		...
		return Colour.positive + "Sold " + amount + " "
				+ GeneralMessages.material(item) + " for "
				+ coins(price * amount) + ".";
	...
	public static String bought(Material item, Integer amount, Double price) {
		...
		return Colour.positive + "Bought " + amount + " "
				+ GeneralMessages.material(item) + " for "
				+ coins(price * amount) + ".";
	...
	public static String gotPaid(Faction faction, Double amount) {
		return faction.getColour2() + "Received " + coins(amount)
				+ " in wages.";
	}
	...
	public static String gotKillReward(SagaPlayer attakerPlayer,
			SagaPlayer defenderPlayer, Faction faction, Double amount) {
		return faction.getColour2() + attakerPlayer.getName() + " received "
				+ coins(amount) + " for killing " + defenderPlayer.getName()
				+ ".";
	}
	...
	public static String exported(ArrayList<SagaItem> exports, Double coins) {
		...
		for (int i = 0; i < exports.size(); i++) {
			...
			if (i != 0 && exports.get(i - 1).getType() == item.getType())
				duplic = true;
			if (i != exports.size() - 1
					&& exports.get(i + 1).getType() == item.getType())
				duplic = true;
