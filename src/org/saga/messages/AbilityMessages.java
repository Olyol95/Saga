...
public class AbilityMessages {
	...
	// Usage:
	public static String insufficientItems(Ability ability, Material material,
			Integer amount) {
		return Colour.negative + ChatUtil.capitalize(ability.getName())
				+ " requires " + amount + " "
	...
	public static String onCooldown(Ability ability) {
		return Colour.negative + ChatUtil.capitalize(ability.getName())
				+ " is on cooldown for " + ability.getCooldown() + "s.";
	}
	...
	public static String insuficientEnergy(String name) {
		return Colour.negative + "Not enough energy to use " + name
				+ " ability.";
	}
	...
	public static String repairAlreadyRepaired(Material material) {
		...
		return Colour.negative
				+ ChatUtil.capitalize(GeneralMessages.material(material))
				+ " is already repaired.";
	...
	public static String repairLevelsRequired(Integer levels) {
		...
		if (levels == 1) {
			return Colour.negative + levels.toString()
					+ " enchant level required.";
		}
		return Colour.negative + levels.toString()
				+ " enchant levels required.";
	...
	public static String digTargetDirtSand(Ability ability) {
		...
		return Colour.negative + ChatUtil.capitalize(ability.getName())
				+ " can only be used on dirt or sand.";
	...
	public static String trimNotStroungEnough(Ability ability, int maximum) {
		return Colour.negative + "Not strong enough to trim more than "
				+ maximum + " blocks.";
	}
	...
	public static String chopDownNotTree(Ability ability) {
		return Colour.negative + ChatUtil.capitalize(ability.getName())
				+ " ability can only be used on trees.";
	}

	public static String chopDownNotStroungEnough(Ability ability, int maximum) {
		return Colour.negative
				+ "Not strong enough to chop a tree larger than " + maximum
				+ " blocks.";
	}
