...
public class BundleManager {
	...
	public void syncBundle(SagaPlayer sagaPlayer) {
		...
		if (bundle == null) {
			SagaLogger.severe(getClass(), "bundle " + bundleId
		...
		if (!bundle.isMember(sagaPlayer.getName())) {
			SagaLogger.severe(getClass(), "player " + sagaPlayer.getName()
					+ " is not on the member list for bundle " + bundle);
			sagaPlayer.removeBundleId();
			return;
		}
	...
	public Bundle getBundle(String name) {
		...
		for (Bundle bundle : bundles) {
			if (bundle.getName().toLowerCase().equalsIgnoreCase(name))
				return bundle;
		}
	...
	public Bundle matchBundle(String name) {
		...
		for (Bundle matchBundle : factions) {
			if (matchBundle.getName().toLowerCase()
	...
	public SagaChunk getSagaChunk(Chunk chunk) {
		...
		Hashtable<Integer, Hashtable<Integer, SagaChunk>> world = sagaChunks
	...
	public SagaChunk getSagaChunk(String worldName, int x, int z) {
		...
		Hashtable<Integer, Hashtable<Integer, SagaChunk>> tworld = sagaChunks
				.get(worldName);
		if (tworld == null) {
			return null;
		}
	...
	void removeSagaChunk(SagaChunk sagaChunk) {
		...
		if (world == null) {
			SagaLogger.severe(this, "tried to remove a non-existan "
					+ sagaChunk + " chunk to group manager chunk shortcut");
			return;
		}
	...
	void addSagaChunk(SagaChunk sagaChunk) {
		...
		if (z != null) {
			SagaLogger.severe(getClass(), "overlap between " + z + " and "
					+ sagaChunk + " Saga chunks");
			return;
		}
	...
	void addBundle(Bundle bundle) {
		...
		if (registeredBundles.containsKey(bundle.getId())) {
			SagaLogger.severe(getClass(), "tried to add an already existing "
					+ bundle + " chunk group");
			return;
		}
	...
	public static void load() {
		...
		for (String id : ids) {
			...
			if (element.getId() < 0) {
				SagaLogger.severe(BundleManager.class, "can't load " + element
						+ " chunk group, because it has an invalid ID");
				continue;
			}
