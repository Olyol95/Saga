...
public class SagaMap {
	...
	public static ArrayList<String> getMap(SagaPlayer sagaPlayer,
			...
			for (int dx = 0; dx < height; dx++) {
				...
				if (sagaChunk != null) {
					...
					if (sagaChunk.getBuilding() != null) {
						symbol = BUILDING.replace("B", sagaChunk.getBuilding()
								.getMapChar());
				...
				// Not claimed:
					...
					Biome biome = location.getWorld().getBiome(
							(topLeftX + dx) * 16 + 8, (topLeftZ + dz) * 16 + 8);
		...
		ArrayList<String> asciiCompass = AsciiCompass.getAsciiCompass(
				inDegrees, ChatColor.RED, ChatColor.GOLD);
		...
		for (int i = 0; i + 4 < map.size() && i < locationName.length; i++) {
			map.set(i + 4, map.get(i + 4) + "   " + ChatColor.GOLD
					+ locationName[i]);
		}
