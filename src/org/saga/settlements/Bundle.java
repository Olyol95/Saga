...
public class Bundle extends SagaCustomSerialization {
	...
	public void delete() {
		...
		for (String member : members) {
			...
			try {
				...
				SagaLogger.severe(this, "failed to remove " + member
						+ " player");
	...
	public boolean checkDetele() {
		...
		// Too big:
		if (groupChunks.size() >= SettlementConfiguration.config()
				.getNoDeleteSize())
			return false;
	...
	/**
	 * Adds a chunk. Needs to be done by chunk group manager, to update chunk
	 * shortcuts.
	 * 
	 * @param sagaChunk
	 *            saga chunk
	...
	public void addChunk(SagaChunk sagaChunk) {
		...
		if (groupChunks.contains(sagaChunk)) {
			SagaLogger.severe(this, "tried to add an already existing "
					+ sagaChunk + "chunk");
			return;
		}
	...
	/**
	 * Removes a chunk. Needs to be done by chunk group manager, to update chunk
	 * shortcuts.
	 * 
	 * @param sagaChunk
	 *            saga chunk
	...
	public void removeChunk(SagaChunk sagaChunk) {
		...
		if (!groupChunks.contains(sagaChunk)) {
			SagaLogger.severe(this, "tried to remove a non-existing "
					+ sagaChunk + "chunk");
			return;
		}
	...
	public ArrayList<Building> getBuildings() {
		...
		if (buildings == null) {
			...
			Collections.sort(buildings, BuildingConfiguration.config()
					.getComparator());
	...
	public Integer getRemainingBuildings(String buildingName) {
		...
		return getAvailableBuildings(buildingName)
				- getTotalBuildings(buildingName);
	 ...
	 * @return buildings that are instances of the given class
	 */
	public <T extends Building> ArrayList<T> getBuildings(Class<T> bldClass,
			String bldgName) {
		...
		for (Building building : allBuildings) {
			...
			if (bldClass.isInstance(building)
					&& building.getName().equalsIgnoreCase(bldgName)) {
				try {
	...
	public void information(String message, SagaPlayer member) {
		...
		message = Colour.normal2 + "(" + Colour.normal1 + "info"
	...
	public void information(Building building, String message, SagaPlayer member) {
		...
		message = Colour.normal2 + "(" + Colour.normal1 + building.getName()
				+ Colour.normal2 + ") " + message;
	...
	void onCreatureSpawn(CreatureSpawnEvent event, SagaChunk locationChunk) {
		...
		for (int i = 0; i < groupChunks.size() && !event.isCancelled(); i++) {
			...
			if (building != null)
				building.onCreatureSpawn(event, locationChunk);
		}
	 ...
	 *            location chunk
	 */
	public void onPlayerCommandPreprocess(SagaPlayer sagaPlayer,
			PlayerCommandPreprocessEvent event, SagaChunk sagaChunk) {

		String command = event.getMessage().split(" ")[0].replace("/", "");

		// Permission:
		if (SettlementConfiguration.config().checkMemberOnlyCommand(command)
				&& !hasPermission(sagaPlayer,
						SettlementPermission.MEMBER_COMMAND)) {
			sagaPlayer.message(GeneralMessages.noCommandPermission(this,
					command));
			event.setCancelled(true);
			return;
		}
	...
	public void onPlayerInteract(PlayerInteractEvent event,
			SagaPlayer sagaPlayer, SagaChunk sagaChunk) {
		...
		if (item != null && item.getType() == Material.POTION) {
			...
			if (GeneralConfiguration.config().getHarmfulSplashPotions()
					.contains(durability)) {
	...
	public void onMemberRespawn(SagaPlayer sagaPlayer, PlayerRespawnEvent event) {
		...
		for (SagaChunk sagaChunk : groupChunks) {
			...
			Building building = sagaChunk.getBuilding();
			if (building != null)
				building.onMemberRespawn(sagaPlayer, event);
	...
	public static Bundle load(String id) {
		...
		try {
			...
			bundle = WriterReader.read(Directory.SETTLEMENT_DATA, id,
					Bundle.class);
			...
			SagaLogger.severe(Bundle.class, "failed to read data for " + id
					+ " ID");
			bundle = new Bundle("invalid");
			...
			SagaLogger.severe(Bundle.class, "failed to parse data for " + id
