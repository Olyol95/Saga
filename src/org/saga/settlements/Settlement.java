...
public class Settlement extends Bundle implements MinuteTicker, DaytimeTicker {
	...
	public Settlement(String name) {
		...
		super(name);
		level = 0;
		claims = SettlementConfiguration.config().getInitialClaims()
				.doubleValue();
		buildPoints = SettlementConfiguration.config().getInitialBuildPoints()
				.doubleValue();
		coins = 0.0;
	...
	public void complete() {
		...
		if (claims == null && level != null) {
			...
			claims = SettlementConfiguration.config().getMaxClaims()
					.doubleValue()
					* level.doubleValue() / maxLevel;
			SagaLogger.nullField(this, "claims");
		}

		// TODO: Remove building points conversion:
		if (buildPoints == null) {
			buildPoints = SettlementConfiguration.config()
					.getBuildPoints(getSize()).doubleValue();
		}
		...
		while (playerNames.hasMoreElements()) {
			...
			try {
				...
				SagaLogger.severe(this, "tried to add an invalid "
						+ proficiency + " role:" + e.getMessage());
		...
		try {
			...
			for (String member : members) {
				...
				if (getRole(member) != null)
					continue;

				Proficiency role = ProficiencyConfiguration.config()
						.createProficiency(
			...
			SagaLogger.severe(this, "failed to set "
					+ SettlementConfiguration.config().getDefaultRole()
					+ " role, because the role name is invalid");
		}
	...
	public static void create(Settlement settlement, SagaPlayer owner) {
		...
		// try {
		// Proficiency role =
		// ProficiencyConfiguration.config().createProficiency(settlement.getDefinition().ownerRole);
		// settlement.setRole(owner, role);
		// } catch (InvalidProficiencyException e) {
		// SagaLogger.severe(settlement, "failed to set " +
		// settlement.getDefinition().ownerRole +
		// " role, because the role name is invalid");
		// }
	...
	public boolean isRoleAvailable(String roleName) {
		...
		ProficiencyDefinition role = ProficiencyConfiguration.config()
				.getDefinition(roleName);
		if (role == null)
			return false;
	...
	public Double takeWorkPoints(String roleName, Double requested) {
		...
		if (available - requested > 0)
			workPoints.put(roleName, available - requested);
		else
			workPoints.remove(roleName);
	...
	public void distribWages(Double paid) {
		...
		for (SagaPlayer member : online) {
			...
			wageWeigths[i] = EconomyConfiguration.config().getWageWeigth(this,
					member);
	...
	public Integer getTotalClaims() {
		if (claims > SettlementConfiguration.config().getMaxClaims())
			return SettlementConfiguration.config().getMaxClaims();
	...
	public boolean isClaimsAvailable() {
		return getAvailableClaims() > 0
				|| isOptionEnabled(BundleToggleable.UNLIMITED_CLAIMS);
	}
	...
	public void payCoins(Double amount) {
		...
		Double settlementShare = amount
				* EconomyConfiguration.config().getSettlementPercent();
		Double factionShare = amount
				* EconomyConfiguration.config().getSettlementFactionPercent();
		Double membersShare = amount
				* EconomyConfiguration.config().getSettlementMemberPercent();
		...
		// Faction:
		Faction owningFaction = SiegeManager.manager()
				.getOwningFaction(getId());
		if (owningFaction != null)
			owningFaction.payCoins(factionShare);
	...
	public Integer getAvailableBuildings(String buildingName) {
		...
		BuildingDefinition definition = BuildingConfiguration.config()
				.getBuildingDefinition(buildingName);
	 ...
	 * 
	 * @see
	 * org.saga.chunkGroups.ChunkBundle#hasPermisson(org.saga.player.SagaPlayer,
	 * org.saga.settlements.Settlement.SettlementPermission)
	 */
	public boolean hasPermission(SagaPlayer sagaPlayer,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.saga.chunks.Bundle#chat(org.saga.player.SagaPlayer,
	 * java.lang.String)
	 */
	@Override
	public void chat(SagaPlayer sagaPlayer, String message) {

		message = Colour.normal2 + "[" + Colour.normal1
				+ SettlementMessages.roledPlayer(this, sagaPlayer)
				+ Colour.normal2 + "] " + message;
	...
	public Date getLastSeen(String name) {
		...
		// Online:
		if (Saga.plugin().isSagaPlayerLoaded(name))
			return Calendar.getInstance().getTime();

		// Offline:
		Date lastDate = lastSeen.get(name);
		if (lastDate == null) {
			SagaLogger.severe(this, "last seen date not found for " + name
					+ " player");
	...
	public boolean isMemberActive(String name) {
		...
		inactiveCal.add(Calendar.DAY_OF_MONTH,
				-SettlementConfiguration.config().inactiveSetDays);
	...
	public boolean checkActiveMembers() {
		return countActiveMembers() >= SettlementConfiguration.config()
				.getRequiredActiveMembers(getSize());
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.saga.chunkGroups.ChunkBundle#onBuild(org.saga.listeners.events.
	 * SagaBuildEvent)
	 */
	@Override
	public void onBuild(SagaBuildEvent event) {
		...
		// Add override:
		if (!hasPermission(event.getSagaPlayer(), SettlementPermission.BUILD))
			event.addBuildOverride(BuildOverride.SETTLEMENT_DENY);
	 ...
	 * 
	 * @see
	 * org.saga.chunkGroups.ChunkBundle#onMemberQuit(org.bukkit.event.player
	 * .PlayerQuitEvent, org.saga.player.SagaPlayer)
	 */
	@Override
	...
	public boolean clockMinuteTick() {
		...
		if (checkRequirements()) {
			...
			if (claims < SettlementConfiguration.config().getMaxClaims()) {
				...
				claims += SettlementConfiguration.config().getClaimsPerMinute(
						online);

				// Statistics:
				StatisticsManager.manager().addSettlementClaims(
						this,
						SettlementConfiguration.config().getClaimsPerMinute(
								online));

			}

			// Increase build points:
			if (buildPoints < SettlementConfiguration.config()
					.getMaxBuildPoints()) {

				buildPoints += SettlementConfiguration.config()
						.getBuildPointsPerMinute(online);

				// Statistics:
				StatisticsManager.manager().addSettlementBuildPoints(
						this,
						SettlementConfiguration.config()
								.getBuildPointsPerMinute(online));
	 ...
	 * 
	 * @see
	 * org.saga.Clock.DaytimeTicker#daytimeTick(org.saga.Clock.DaytimeTicker
	 * .Daytime)
	 */
	@Override
	...
	public void handleWork() {
		...
		for (String role : onlineRoles) {
			if (workPoints.get(role) > onlineRolesTotals.get(role))
				workPoints.put(role, onlineRolesTotals.get(role));
		}
	...
	public enum SettlementPermission {
		...
		TOGGLE_UNLIMITED_STORAGE,

		// General:
		ACCESS_WAREHOUSE, ACCESS_STORAGE, ADD_COINS, OPEN_HOME_CONTAINERS, OPEN_SETTLEMENT_CONTAINERS, BUILD, BUILD_BUILDING, BUILDING_UPGRADE, BUY_CLAIMS, BUY_BUILD_POINTS, CLAIM, CLAIM_SETTLEMENT, CRUMBLE_ARENA_SETUP, ABANDON, DECLARE_OWNER, DISSOLVE, FLASH_CHUNK, INVITE, KICK, SET_AFFILIATION, SET_ROLE, SET_BUILDING, TOWN_SQUARE_SET_SPAWN, REMOVE_AFFILIATION, REMOVE_BUILDING, REMOVE_COINS, RENAME, RESIGN, MEMBER_COMMAND, SPAWN, STORAGE_AREA_ADD, STORAGE_AREA_REMOVE, STORAGE_AREA_FLASH,
