...
public class Clock implements Runnable {
			...
			if (currDaytime != Daytime.NONE && prevDaytime != currDaytime) {
				...
				for (DaytimeTicker ticker : daytimes) {
					...
					if (ticker.checkWorld(worldName)) {
						...
						if (!ticker.daytimeTick(currDaytime))
							this.daytimes.remove(ticker);
	...
	public void enableSecondTick(SecondTicker ticker) {
		...
		synchronized (seconds) {
			...
			if (seconds.contains(ticker)) {
				SagaLogger.warning(getClass(), ticker.getClass()
						.getSimpleName()
						+ "{"
						+ ticker
						+ "}"
						+ " second ticker already registered");
				return;
			}
			seconds.add(ticker);
		...
		synchronized (minutes) {
			...
			if (minutes.contains(ticker)) {
				SagaLogger.warning(getClass(), ticker.getClass()
						...
						+ ticker
						+ "}"
						+ " minute ticker already registered");
				return;
			}
			minutes.add(ticker);
		...
		synchronized (hours) {
			...
			if (hours.contains(ticker)) {
				SagaLogger.warning(getClass(), ticker.getClass()
						...
						+ ticker
						+ "}"
						+ " hour ticker already registered");
				return;
			}
			hours.add(ticker);
		...
		synchronized (daytimes) {
			...
			if (daytimes.contains(ticker)) {
				SagaLogger.warning(getClass(), ticker.getClass()
						...
						+ ticker
						+ "}"
						+ " daytime ticker already registered");
				return;
			}
			daytimes.add(ticker);
	...
	public static void load() {
		...
		clock.taskId = Saga.plugin().getServer().getScheduler()
				.scheduleSyncRepeatingTask(Saga.plugin(), clock, 200L, 20L);
		instance = clock;

		// Current daytimes:
		List<World> worlds = Saga.plugin().getServer().getWorlds();
		for (World world : worlds) {
			clock.prevDaytimes.put(world.getName(),
					Daytime.getDaytime(world.getTime()));
		}
	...
	public interface DaytimeTicker {
		...
		enum Daytime {
			...
			public static Daytime getDaytime(Long time) {
				...
				// Midday:
				if (time > MIDDAY.getTime()
						&& time < MIDDAY.getTime() + LAG_COMPENSATION) {
					return MIDDAY;
				}

				// Sunset:
				if (time > SUNSET.getTime()
						&& time < SUNSET.getTime() + LAG_COMPENSATION) {
					return SUNSET;
				}

				// Midnight:
				if (time > MIDNIGHT.getTime()
						&& time < MIDNIGHT.getTime() + LAG_COMPENSATION) {
					return MIDNIGHT;
				}

				if (time < SUNRISE.getTime() - 24000 + LAG_COMPENSATION)
					time += 24000;

				// Sunrise:
				if (time > SUNRISE.getTime()
						&& time < SUNRISE.getTime() + LAG_COMPENSATION) {
					return SUNRISE;
				}
