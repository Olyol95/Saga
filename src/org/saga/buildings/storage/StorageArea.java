...
public class StorageArea {
	 ...
	 *             when the anchor point is invalid
	 */
	...
	public boolean checkOverlap(StorageArea otherStoreArea) {
		...
		for (Block block : blocks) {
			...
			if (otherStoreArea.checkBelongs(block))
				return true;
	...
	public ItemStack withdrawBlock(ItemStack fromStore, Integer amount) {
		...
		// Withdraw block:
		ArrayList<Block> blocks = SHAPE.getBlocks(anchor.getLocation(),
				orientation, size, FULL_FILTER);

		for (int i = blocks.size() - 1; i >= 0; i--) {

			Block storeBlock = blocks.get(i);

			if (storeBlock.getType() != fromStore.getType())
				continue;

			if (fromStore.getAmount() >= amount)
				break;
	...
	public ItemStack storeItem(ItemStack toStore) {
		...
		for (Chest chest : chests) {
			...
			ItemStack remaining = chest.getBlockInventory().addItem(toStore)
					.get(0);
	...
	public ItemStack withdrawItem(ItemStack fromStore, Integer amount) {
		...
		for (Chest chest : chests) {
			...
			ItemStack remaining = chest.getBlockInventory()
					.removeItem(toRemove).get(0);
	...
	public Integer countStored(ItemStack item) {
		...
		for (Block block : blocks) {
			...
			// Blocks:
				...
				// Data:
				if (block.getData() != item.getData().getData())
					continue;
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof StorageArea) {
			...
			return ((StorageArea) obj).anchor.getLocation().getBlock()
					.equals(anchor.getLocation().getBlock());
