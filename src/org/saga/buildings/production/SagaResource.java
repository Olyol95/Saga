...
public class SagaResource extends SagaRecipe {
	...
	/**
	 * Increases all percentages for free items. Free items have
	 * {@link Material#AIR} recipes.
	 * 
	 */
	public void offerFree() {

		for (int i = 0; i < recipe.length; i++) {
			if (recipe[i].getType() == Material.AIR)
				collected[i] += 1.0;
			if (collected[i] > recipe[i].amount)
				collected[i] = recipe[i].amount;
		}
	 ...
	 * 
	 * @see
	 * org.saga.buildings.production.SagaRecipe#checkAccept(org.saga.buildings
	 * .production.SagaItem, int)
	 */
	@Override
	public boolean checkAccept(SagaItem item, int index)
			throws IndexOutOfBoundsException {

		if (collected[index] >= recipe[index].getAmount())
			return false;
	 ...
	 */
	private void collect(SagaItem item, int index)
			throws IndexOutOfBoundsException {

		double mod = item.getAmount();
		if (mod + collected[index] > recipe[index].getAmount())
			mod = recipe[index].getAmount() - collected[index];
	...
	public double findProducePercentage() {
		...
		for (int i = 0; i < collected.length; i++) {
			if (collected[i] / recipe[i].getAmount() < percent)
				percent = collected[i] / recipe[i].getAmount();
		}
