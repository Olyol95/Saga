...
public class ProductionBuilding extends Building {
	...
	private void synchResources() {
		...
		if (recipes.length != resources.length
				|| recipes.length != buffers.length) {
	...
	public void produce() {
		...
		if (storeable.size() != 0)
			getChunkBundle().information(this,
					BuildingMessages.produced(storeable));
	...
	public void storeBlock(SagaItem sagaItem) {
		...
		while (sagaItem.getAmount() > 0 && possibleStorage.size() > 0) {
			...
			if (possibleStorage.size() == 0)
				possibleStorage = findLowestEmpty();
