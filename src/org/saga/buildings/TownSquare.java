...
public class TownSquare extends Building implements SecondTicker {
	...
	public void updateSpawningProtect(SagaPlayer sagaPlayer) {
		...
		if (previous == null)
			previous = current
					- getDefinition().getFunction(RESPAWN_DAMAGE_IMMUNITY_TIME)
							.getXMax();
		...
		long addition = getDefinition().getFunction(
				RESPAWN_DAMAGE_IMMUNITY_TIME).longValue(passed / 1000.0) * 1000;
	 ...
	 * 
	 * @param spawn
	 *            the spawnLocation to set, null if none
	 */
	...
	public Location findTopSpawnLocation() {
		...
		if (spawnLocation.getY() < 10) {
			SagaLogger.severe(this, spawnLocation
					+ " is an invalid spawn location");
			return null;
		}
	...
	public boolean fillAdjecentValid(Block anchor, HashSet<Block> blocks) {
		...
		// Limit:
		if (blocks.size() > SPAWN_BLOCK_LIMIT)
			return false;
		...
		if (same.getRelative(BlockFace.NORTH).getChunk() == anchor.getChunk())
			if (!fillAdjecentValid(same.getRelative(BlockFace.NORTH), blocks))
				if (!fillAdjecentValid(down.getRelative(BlockFace.NORTH),
						blocks))
		...
		if (same.getRelative(BlockFace.SOUTH).getChunk() == anchor.getChunk())
			if (!fillAdjecentValid(same.getRelative(BlockFace.SOUTH), blocks))
				if (!fillAdjecentValid(down.getRelative(BlockFace.SOUTH),
						blocks))
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.saga.buildings.Building#memberRespawnEvent(org.saga.SagaPlayer,
	...
	public void onMemberRespawn(SagaPlayer sagaPlayer, PlayerRespawnEvent event) {
		...
		if (locationChunk == null) {
			SagaLogger
					.severe(this,
							"can't continue with memberRespawnEvent, because the location chunk isn't set.");
			return;
		}
		...
		if (spawnLocation == null) {
			SagaLogger
					.severe(this,
							"can't continue with onMemberRespawnEvent, because the location can't be retrieved");
	...
	public void onDamage(SagaDamageEvent event) {
		...
		if (event.sagaAttacker instanceof SagaPlayer)
			attackerPlayer = (SagaPlayer) event.sagaAttacker;
