...
public class TrainingCamp extends Building {
	...
	protected boolean isBuildingSign(String firstLine) {
		...
		if (firstLine.equalsIgnoreCase(AttributeSign.SIGN_NAME))
			return true;
		if (firstLine.equalsIgnoreCase(AbilitySign.SIGN_NAME))
			return true;
		if (firstLine.equalsIgnoreCase(ResetAttributeSign.SIGN_NAME))
			return true;
		if (firstLine.equalsIgnoreCase(ResetAbilitySign.SIGN_NAME))
			return true;
	...
	protected BuildingSign createBuildingSign(Sign sign, SignChangeEvent event) {
		...
		if (event.getLine(0).equalsIgnoreCase(AttributeSign.SIGN_NAME)) {
			return AttributeSign.create(sign, event.getLine(1),
					event.getLine(2), event.getLine(3), this);
		}

		// Ability sign:
		if (event.getLine(0).equalsIgnoreCase(AbilitySign.SIGN_NAME)) {
			return AbilitySign.create(sign, event.getLine(1), event.getLine(2),
					event.getLine(3), this);
		}

		// Reset attribute sign:
		if (event.getLine(0).equalsIgnoreCase(ResetAttributeSign.SIGN_NAME)) {
			return ResetAttributeSign.create(sign, event.getLine(1),
					event.getLine(2), event.getLine(3), this);
		}

		// Reset ability sign:
		if (event.getLine(0).equalsIgnoreCase(ResetAbilitySign.SIGN_NAME)) {
			return ResetAbilitySign.create(sign, event.getLine(1),
					event.getLine(2), event.getLine(3), this);
		}
