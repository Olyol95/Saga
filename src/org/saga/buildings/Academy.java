...
public class Academy extends ProductionBuilding {
	...
	public boolean isBuildingSign(String firstLine) {
		...
		if (GeneralConfiguration.config().isRuneEnabled()
				&& firstLine.equalsIgnoreCase(GuardianRuneSign.SIGN_NAME))
			return true;
	 ...
	 * 
	 * @see
	 * org.saga.buildings.Building#createBuildingSign2(org.bukkit.block.Sign,
	...
	protected BuildingSign createBuildingSign(Sign sign, SignChangeEvent event) {
		...
		if (event.getLine(0).equalsIgnoreCase(GuardianRuneSign.SIGN_NAME)) {
			...
			return GuardianRuneSign.create(sign, event.getLine(1),
