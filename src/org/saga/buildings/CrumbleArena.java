...
public class CrumbleArena extends Building implements SecondTicker {
	...
	public boolean complete() throws InvalidBuildingException {
		...
		if (kickLocation != null)
			try {
				...
				SagaLogger.severe(this, "invalid kick location: "
						+ kickLocation);
			}
	...
	private Integer getRoundDuration() {
		return getDefinition().getFunction(TURN_DURATION_KEY).intValue(
				getMaxRounds() - rounds);
	}
	...
	private ArrayList<Block> filterStableAvailableBlocks(ArrayList<Block> blocks) {
		...
		for (Block block : blocks) {
			...
			if (getAdjacent(block, BlockFace.NORTH).getType() != Material.AIR
					&&
					// getAdjacent(block,BlockFace.NORTH_EAST).getType() !=
					// Material.AIR &&
					getAdjacent(block, BlockFace.EAST).getType() != Material.AIR
					&&
					// getAdjacent(block,BlockFace.SOUTH_EAST).getType() !=
					// Material.AIR &&
					getAdjacent(block, BlockFace.SOUTH).getType() != Material.AIR
					&&
					// getAdjacent(block,BlockFace.SOUTH_WEST).getType() !=
					// Material.AIR &&
	...
	private void createHotsports(ArrayList<Block> blocks) {
		...
		Integer hotspots = getDefinition().getFunction(HOTSPOTS_KEY).intValue(
				getMaxRounds() - rounds);
	...
	public boolean clockSecondTick() {
		...
		// Countdown:
		if (countdown > 0) {
			getSagaChunk().broadcast(
					BuildingMessages.countdown(this, countdown));
			countdown--;
			return true;
		} else if (countdown == 0) {
			getSagaChunk().broadcast(
		...
		// Blocks:
		ArrayList<Block> blocks = getAllArenaBlocks();
		Integer damage = getDefinition().getFunction(DAMAGE_BLOCKS_KEY)
				.intValue(getMaxRounds() - rounds);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.saga.buildings.Building#onSignChange(org.bukkit.event.block.
	 * SignChangeEvent, org.saga.SagaPlayer)
	 */
	@Override
	public void onSignChange(SignChangeEvent event, SagaPlayer sagaPlayer) {
		...
		if (event.getLine(0).equalsIgnoreCase(TOP_SIGN)) {
			event.setLine(0, SettlementConfiguration.config().enabledSignColor
					+ TOP_SIGN);
		}
	 ...
	 * 
	 * @see
	 * org.saga.buildings.Building#onPlayerInteract(org.bukkit.event.player.
	 * PlayerInteractEvent, org.saga.SagaPlayer)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.saga.buildings.Building#onBuild(org.saga.listeners.events.SagaBuildEvent
	 * )
	 */
	@Override
	public void onBuild(SagaBuildEvent event) {
		...
		if (event.getBlock() != null && event.getBlock().getY() == y
				&& !event.getSagaPlayer().isAdminMode()) {
	...
	public static class CrumblePlayer implements Comparable<CrumblePlayer> {
		 ...
		 *            points
		 */
		public CrumblePlayer(String name, Integer wins, Integer losses,
				Double points) {
		...
		public int compareTo(CrumblePlayer o) {
			...
			return new Double((o.getWins() - o.getLosses())
					- (getWins() - getLosses())).intValue();
