...
public class Arena extends Building implements SecondTicker {
	...
	public boolean clockSecondTick() {
		...
		if (sagaChunk != null)
			sagaChunk.broadcast(BuildingMessages.countdown(count));
	 ...
	 * 
	 * @see
	 * org.saga.buildings.Building#onPlayerDamagedByPlayer(org.bukkit.event.
	 * entity.EntityDamageByEntityEvent, org.saga.SagaPlayer,
	 * org.saga.SagaPlayer)
	 */
	@Override
	public void onDamage(SagaDamageEvent event) {
		...
		if (attackerChunk == null || defenderChunk == null)
			return;
		if (attackerChunk.getBuilding() == null
				|| !(attackerChunk.getBuilding() instanceof Arena))
			return;
		if (defenderChunk.getBuilding() == null
				|| !(defenderChunk.getBuilding() instanceof Arena))
			return;
	 ...
	 * 
	 * @see
	 * org.saga.buildings.Building#onPlayerKillPlayer(org.saga.player.SagaPlayer
	...
	public void onPvPKill(SagaPlayer attacker, SagaPlayer defender) {
		...
		// Points:
		addPoints(attacker.getName(), defender.getUsedAttributePoints()
				.doubleValue() * ATTRIBUTE_MULTIPLIER);
	...
	private void synchArenas() {
		...
		if (this.count > 0) {
			for (Arena arena : arenas) {
				if (arena != this)
					arena.intiateCountdown(this.count);
			}
		}
