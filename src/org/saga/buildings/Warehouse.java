...
public class Warehouse extends Building {
	...
	// Store:
	/**
	 * Stores the item. Given item will be modified accordingly.
	 * 
	 * @param sagaItem
	 *            saga item
	 */
	...
	private void storeBlock(SagaItem sagaItem) {
		...
		for (int i = 0; i < buffer.size(); i++) {
			...
			if (i == buffer.size() - 1)
				buffer.add(new SagaItem(buffer.get(i)));
	...
	public SagaItem withdraw(SagaItem requestedItem) {
		...
		if (requestedItem.getType().isBlock())
			return withdrawBlock(requestedItem);
		else
			return withdrawItem(requestedItem);
	...
	private SagaItem withdrawBlock(SagaItem requestedItem) {
		...
		if (requestedItem.getAmount() < 1.0)
			return removedItem;
		...
		for (int i = 0; i < buffer.size(); i++) {
			...
			if (buffer.get(i).checkRepresents(removedItem)) {
				...
				double mod = requestedItem.getAmount()
						- removedItem.getAmount();

				buffer.get(i).modifyAmount(-mod);
				removedItem.modifyAmount(mod);

				if (buffer.get(i).getAmount() <= 0.0)
					buffer.remove(i);
		...
		if (getSagaChunk().isLoaded())
			for (Block block : possibleStorage) {
				...
				if (!requestedItem.checkRepresents(block))
					continue;
	...
	private SagaItem withdrawItem(SagaItem requestedItem) {
		...
		for (Chest chest : possibleStorage) {
			...
			int mod = (int) (requestedItem.getAmount() - removedItem
					.getAmount());
	...
	public double count(SagaItem countItem) {
		...
		if (countItem.getType().isBlock())
			return countBlock(countItem);
		else
			return countItem(countItem);
	...
	public double countBlock(SagaItem countItem) {
		...
		if (getSagaChunk().isLoaded())
			for (Block block : possibleStorage) {
				...
				if (!countItem.checkRepresents(block))
					continue;
		...
		for (StorageArea storageArea : storages) {
			...
			for (Block block : blocks) {
				if (block.getState() instanceof Chest)
					possible.add((Chest) block.getState());
			}
	 ...
	 * PlayerInteractEvent, org.saga.player.SagaPlayer)
	 */
	@Override
	public void onPlayerInteract(PlayerInteractEvent event,
		...
		if (!bundle.hasPermission(sagaPlayer,
				SettlementPermission.ACCESS_WAREHOUSE)) {

			// Chest:
			if (targetMaterial.equals(Material.CHEST)
			...
			// Trapped chest:
			if (targetMaterial.equals(Material.TRAPPED_CHEST)
					&& event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
			...
			// Furnace:
			else if (targetMaterial.equals(Material.FURNACE)
					&& event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
			...
			// Dispenser:
			else if (targetMaterial.equals(Material.DISPENSER)
					&& event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
