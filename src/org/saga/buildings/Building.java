 ...
 * 
 */
public abstract class Building extends SagaCustomSerialization implements
		DaytimeTicker {
	 ...
	 *             if the building definition is missing
	 */
	public boolean complete() throws InvalidBuildingException {
		...
		for (int i = 0; i < signs.size(); i++) {
			...
			try {
				...
				SagaLogger.severe(
						this,
						"failed to initialise signs field element: "
		...
		for (int i = 0; i < storage.size(); i++) {
			...
			try {
				...
				SagaLogger.severe(
						this,
						"failed to initialise storage field element: "
								+ e.getClass().getSimpleName() + ":"
								+ e.getMessage());
				storage.remove(i);
				i--;
		...
		if (!get_className().equals(definition.getBuildingClass())) {
			SagaLogger.severe(this,
					"building and definition className fields don't match");
			SagaLogger.info(this, "_className=" + get_className()
					+ ", buildingClass=" + definition.getBuildingClass());
	 ...
	 *            buildings array
	 */
	public <T extends Building> void collectAdjacentBuildings(T anchor,
			HashSet<T> buildings) {

		// Already on the list:
		if (!buildings.add(anchor))
			return;
		...
		if (building != null
				&& building.getClass().isAssignableFrom(anchorClass)) {
	 ...
	 *            sign change event
	 */
	public final void handleSignPlace(SagaPlayer sagaPlayer, Sign sign,
		...
		if (!getChunkBundle().hasPermission(sagaPlayer,
				SettlementPermission.BUILD_BUILDING)) {
			sagaPlayer.message(GeneralMessages.noPermission(this));
			return;
		}

		// Sign create permission:
		SettlementPermission createPermission = getBuildingSignCreatePermission(event
				.getLine(0));
		if (createPermission != null
				&& !getChunkBundle()
						.hasPermission(sagaPlayer, createPermission)) {
			sagaPlayer.message(GeneralMessages.noPermission(this));
			return;
		}
	 ...
	 *            sign remove event
	 */
	public final void handleSignRemove(SagaPlayer sagaPlayer, Sign sign,
			BlockBreakEvent event) {

		// Building sign:
		BuildingSign buildingSign = buildingSignAt(event.getBlock()
				.getLocation());
		if (buildingSign == null)
			return;
		...
		SettlementPermission removePermission = getBuildingSignCreatePermission(sign
				.getLine(0));
		if (removePermission != null
				&& !getChunkBundle()
						.hasPermission(sagaPlayer, removePermission)) {
			sagaPlayer.message(GeneralMessages.noPermission(this));
			return;
		}
	 ...
	 */
	protected SettlementPermission getBuildingSignCreatePermission(
			String firstLine) {
		return null;
	}
	 ...
	 */
	protected SettlementPermission getBuildingSignRemovePermission(
			String firstLine) {
		return null;
	}
	 ...
	 * @return the signs
	 */
	public <T extends BuildingSign> ArrayList<T> getBuildingSigns(
			Class<T> signClass) {
	...
	protected void addBuildingSign(BuildingSign buildingSign) {
		...
		if (signs.contains(buildingSign)) {
			SagaLogger.severe(this,
					"tried to add an already existing building sign");
			return;
		}
	...
	protected void removeBuildingSign(BuildingSign buildingSign) {
		...
		if (!signs.contains(buildingSign)) {
			SagaLogger.severe(this,
					"tried to remove a non-existing building sign");
			return;
		}
	 ...
	 * 
	 * @param otherStoreArea
	 *            storage area to check
	...
	public boolean checkOverlap(StorageArea otherStoreArea) {
		...
		for (StorageArea storageArea : allSorage) {
			...
			if (storageArea.checkOverlap(otherStoreArea))
				return true;
	 ...
	 *            saga player
	 */
	public void handleItemStorageOpen(PlayerInteractEvent event,
			SagaPlayer sagaPlayer) {

		// Permission:
		if (!getChunkBundle().hasPermission(sagaPlayer,
				SettlementPermission.ACCESS_STORAGE)
				&& !getChunkBundle().isOptionEnabled(
						BundleToggleable.OPEN_STORAGE_AREAS)) {
	...
	public void handleStore(BlockPlaceEvent event, SagaPlayer sagaPlayer) {
		...
		// Inform:
		sagaPlayer.message(BuildingMessages.stored(event.getBlock().getType(),
				this));
	...
	public void handleWithdraw(BlockBreakEvent event, SagaPlayer sagaPlayer) {
		...
		// Inform:
		sagaPlayer.message(BuildingMessages.withdrew(
				event.getBlock().getType(), this));

	}

	// Stats and description:
	/**
	 * Gets the building specific stats. Each group as a different element.
	 * 
	...
	public void onPlayerInteract(PlayerInteractEvent event,
		...
		if (event.getClickedBlock().getState() instanceof Sign) {
			...
			BuildingSign sign = buildingSignAt(event.getClickedBlock()
					.getLocation());
			if (sign != null)
				sign.onPlayerInteract(sagaPlayer, event);
	 ...
	 *            event
	 * @param locationChunk
	 *            event location saga chunk
	 */
	public void onCreatureSpawn(CreatureSpawnEvent event,
			SagaChunk locationChunk) {
	...
	public void onBuild(SagaBuildEvent event) {
		...
		if (getChunkBundle() != null
				&& !getChunkBundle().hasPermission(event.getSagaPlayer(),
						SettlementPermission.BUILD_BUILDING))
			event.addBuildOverride(BuildOverride.BUILDING_DENY);
		...
		if (block != null && checkStorageArea(block)) {
			...
			// Storage area deny:
			if (!getChunkBundle().hasPermission(event.getSagaPlayer(),
					SettlementPermission.ACCESS_STORAGE)) {
			...
			if (event.getWrappedEvent() instanceof BlockBreakEvent) {
				...
				// Free storage area:
				BlockBreakEvent wrappedEvent = (BlockBreakEvent) event
						.getWrappedEvent();
				if (getChunkBundle().isOptionEnabled(
						BundleToggleable.OPEN_STORAGE_AREAS)
						&& wrappedEvent.getBlock().getType() != Material.CHEST)
					event.addBuildOverride(BuildOverride.OPEN_STORAGE_AREA_ALLOW);

				// Faction claims:
				if (SiegeManager.manager().getOwningFactionID(
						getChunkBundle().getId()) == event.getSagaPlayer()
						.getFactionId()) {

					// Faction storage area access:
					if (FactionConfiguration.config()
							.isOpenClaimedStorageAreas())
						event.addBuildOverride(BuildOverride.OPEN_CLAIMED_STORAGE_AREA_ALLOW);

					// Faction open access:
					else if (getChunkBundle().isOptionEnabled(
							BundleToggleable.FACTION_STORAGE_ACCESS))
						event.addBuildOverride(BuildOverride.OPEN_CLAIMED_STORAGE_AREA_ALLOW);
	 ...
	 * @return building
	 * @throws Throwable
	 *             thrown if validation fails, message is in the form that can
	 *             be sent to saga player
	 */
	public static <T extends Building> T retrieveBuilding(CommandContext args,
			Saga plugin, SagaPlayer sagaPlayer, Class<T> buildingClass)
			throws Throwable {
		...
		if (building == null) {
			throw new Throwable(BuildingMessages.buildingCommandRestrict(
					ChatUtil.className(buildingClass), args.getCommand()));
		}
