...
public class ExportSign extends BuildingSign {
	 ...
	 *            building
	 */
	protected ExportSign(Sign sign, String secondLine, String thirdLine,
	 ...
	 */
	public static ExportSign create(Sign sign, String secondLine,
	...
	public SignStatus getStatus() {
		...
		if (getBuilding() instanceof TradingPost) {
			...
			if (((TradingPost) getBuilding()).checkOverExportLimit())
	...
	protected void onRightClick(SagaPlayer sagaPlayer) {
		...
		SagaPricedItem exportItem = EconomyConfiguration.config()
				.getExportItem(item);
		...
		sagaPlayer.message(EconomyMessages.sold(item.getType(), takenItem
				.getAmount().intValue(), price));

		// Notify transaction:
		if (getBuilding() instanceof TradingPost) {
			((TradingPost) getBuilding()).notifyExport(price
					* takenItem.getAmount());
		}

		// Statistics:
		StatisticsManager.manager().addExport(sagaPlayer, item.getType(),
				takenItem.getAmount().intValue(), cost);
