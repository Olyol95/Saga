...
public class BuySign extends BuildingSign {
	 ...
	 *            building
	 */
	protected BuySign(Sign sign, String secondLine, String thirdLine,
	 ...
	 */
	public static BuySign create(Sign sign, String secondLine,
	...
	private void initialiseFields() {
		...
		// Check maxStored:
		if (maxStored != null && maxStored <= 0.0)
			maxStored = null;
	...
	public String getLine(int index, SignStatus status) {
		...
		case ENABLED:
			...
			if (index == 1)
				return item.getAmount().intValue() + AMOUNT_DIV_DISPLAY
						+ GeneralMessages.materialAbrev(item.getType());
			if (index == 2)
				return "price: " + EconomyMessages.coins(price);
			if (index == 3)
				return stored.intValue() + "/" + maxStored.intValue();
	...
	protected void onRightClick(SagaPlayer sagaPlayer) {
		...
		// Statistics:
		StatisticsManager.manager().addBuy(sagaPlayer, item.getType(),
				item.getAmount(), price * item.getAmount());
