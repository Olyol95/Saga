...
public class ResetSign extends BuildingSign {
	 ...
	 *            building
	 */
	private ResetSign(Sign sign, String secondLine, String thirdLine,
	 ...
	 */
	public static ResetSign create(Sign sign, String secondLine,
			String thirdLine, String fourthLine, Building building) {
		return new ResetSign(sign, secondLine.toLowerCase(), thirdLine,
				fourthLine, building);
	}
	...
	public String getLine(int index, SignStatus status) {
		...
		case ENABLED:
			...
			if (index == 1)
				return getAmount() + AMOUNT_DIV_DISPLAY + getAttribute();
			if (index == 3 && cost > 0.0)
				return "cost: " + EconomyMessages.coins(cost);
			break;
		...
		case INVALIDATED:
			...
			if (index == 1)
				return SettlementConfiguration.config().invalidSignColor
						+ "amt" + AMOUNT_DIV_DISPLAY + "attribute";
			break;
	...
	protected void onRightClick(SagaPlayer sagaPlayer) {
		...
		// Amount to reset:
		Integer amount = getAmount();
		if (amount > attributeScore)
			amount = attributeScore;
		...
		// Inform:
		sagaPlayer.message(BuildingMessages.resetAttr(attribute, attributeScore
				- amount));
