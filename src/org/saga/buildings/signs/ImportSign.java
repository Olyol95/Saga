...
public class ImportSign extends BuildingSign {
	 ...
	 *            building
	 */
	protected ImportSign(Sign sign, String secondLine, String thirdLine,
	 ...
	 */
	public static ImportSign create(Sign sign, String secondLine,
			String thirdLine, String fourthLine, Building building) {
	...
	public SignStatus getStatus() {
		...
		if (EconomyConfiguration.config().getImportItem(item) == null)
			return SignStatus.INVALIDATED;

		// Import limit:
		if (getBuilding() instanceof TradingPost) {
			if (((TradingPost) getBuilding()).checkOverImportLimit())
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.saga.buildings.signs.BuildingSign#getLine(int,
	 * org.saga.buildings.signs.BuildingSign.SignStatus)
	 */
	@Override
	public String getLine(int index, SignStatus status) {
		...
		case ENABLED:
			...
			if (index == 1)
				return item.getAmount() + AMOUNT_DIV_DISPLAY
						+ GeneralMessages.materialAbrev(item.getType());
			if (index == 2)
				return "price: " + EconomyMessages.coins(price);
		...
		case INVALIDATED:
			...
			if (index == 1)
				return SettlementConfiguration.config().invalidSignColor
						+ "amt" + AMOUNT_DIV_DISPLAY + "item/ID";
	 ...
	 * 
	 * @see
	 * org.saga.buildings.signs.BuildingSign#onRightClick(org.saga.player.SagaPlayer
	 * )
	 */
	@Override
	protected void onRightClick(SagaPlayer sagaPlayer) {

		SagaPricedItem importItem = EconomyConfiguration.config()
				.getImportItem(item);
		if (importItem == null)
			return;
		...
		// Trim based on coins:
		if (price * giveItem.getAmount() > coins)
			giveItem.setAmount((int) (coins / price));
		...
		// Inform:
		sagaPlayer.message(EconomyMessages.bought(item.getType(),
				giveItem.getAmount(), price));

		// Notify transaction:
		if (getBuilding() instanceof TradingPost) {
			((TradingPost) getBuilding()).notifyImport(price
					* giveItem.getAmount());
		}

		// Statistics:
		StatisticsManager.manager().addImport(sagaPlayer, item.getType(),
				giveItem.getAmount(), cost);
