...
public class Saga extends JavaPlugin implements MinuteTicker {
	...
	public void onEnable() {
		...
		// Commands map:
		CommandsManager<Player> commandMap = PermissionsDependency
				.getCommandMap();
	...
	/**
	 * Loads a saga player. If the player doesn't exists, then a new one is
	 * created.
	 * 
	 * @param name
	 *            player name
	...
	public SagaPlayer loadSagaPlayer(String name) {
		...
		if (sagaPlayer != null) {
			SagaLogger.severe(getClass(),
					"tried to load already loadaded Saga player for " + name);
			return sagaPlayer;
		}
	...
	public SagaPlayer unloadSagaPlayer(String name) {
		...
		if (sagaPlayer == null) {
			SagaLogger.severe(getClass(),
					"tried unload a non-loaded player for " + name);
			return sagaPlayer;
		}
	 ...
	 * @param name
	 *            player name
	 * @throws NonExistantSagaPlayerException
	 *             if the player doesn't exist
	 */
	public SagaPlayer forceSagaPlayer(String name)
			throws NonExistantSagaPlayerException {
		...
		try {
			...
			try {
				...
				commandMap.execute(split, player, this,
						getLoadedPlayer(player.getName()));
				SagaLogger.info("[Saga Command] " + player.getName() + ": "
						+ command);
	 ...
	 * @throws SagaPlayerNotLoadedException
	 *             when the player isn't loaded
	 */
	public SagaPlayer matchPlayer(String name)
			throws SagaPlayerNotLoadedException {
