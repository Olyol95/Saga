...
public class GuardianRune {
	...
	public void restore(Player player) {
		...
		try {
			...
			SagaLogger.severe(this, "guardian rune failure: "
	...
	public boolean isEmpty() {
		return levels <= 0 && exp <= 0 && items.length <= 0
				&& armour.length <= 0;
	}
	...
	public static void handleRestore(SagaPlayer sagaPlayer) {
		...
		try {
			...
			if (!GeneralConfiguration.config().isRuneEnabled(
					sagaPlayer.getLocation().getWorld()))
				return;
			...
			StatisticsManager.manager()
					.addGuardRuneRestoreException(sagaPlayer);
		}
	 ...
	 */
	public static boolean handleAbsorb(SagaPlayer sagaDead,
		...
		// Nothing to absorb:
		if (countItems(player.getInventory().getContents()) <= 0
				&& countItems(player.getInventory().getArmorContents()) <= 0
