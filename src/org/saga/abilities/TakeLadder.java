...
public class TakeLadder extends Ability {
	...
	public boolean handleInteractPreTrigger(PlayerInteractEvent event) {
		...
		// Check placement:
		Block targetBlock = event.getPlayer()
				.getTargetBlock((Set<Material>)null, MAX_DISTANCE);
	...
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		for (Block ladderBlock : ladder) {
			...
			// Add ladder:
			InventoryUtil.addItem(new ItemStack(Material.LADDER, 1), inventory,
					player.getLocation());
		...
		if (triggered) {
			...
			getSagaLiving().playGlobalEffect(Effect.STEP_SOUND,
					Material.LADDER.getId(), targetBlock.getLocation());
		}
