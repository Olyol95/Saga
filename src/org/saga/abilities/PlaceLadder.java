...
public class PlaceLadder extends Ability {
	...
	public boolean handleInteractPreTrigger(PlayerInteractEvent event) {
		...
		if (!canSupport(clickedBlock.getRelative(BlockFace.UP), data))
			return false;
	...
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		// Height:
		int height = getDefinition().getFunction(LADDER_HEIGHT_KEY).intValue(
				getScore());
		...
		for (int i = 1; i <= height; i++) {
			...
			// Remove ladder:
			InventoryUtil.removeItem(new ItemStack(Material.LADDER, 1),
					inventory);
			triggered = true;
		...
		// Effect:
		sagaLiving.playGlobalEffect(Effect.STEP_SOUND, Material.LADDER.getId(),
	 ...
	 *            target block
	 * @param sagaLiving
	 *            saga entity that placed the ladder, null if ignore
	...
	private static byte getData(Block target, SagaLiving sagaLiving) {
		...
		if (sagaLiving != null) {
			...
			if (orientation == Orientation.NORTH
					&& canAttach(target.getRelative(BlockFace.NORTH)))
				return 3;
			if (orientation == Orientation.EAST
					&& canAttach(target.getRelative(BlockFace.EAST)))
				return 4;
			if (orientation == Orientation.SOUTH
					&& canAttach(target.getRelative(BlockFace.SOUTH)))
				return 2;
			if (orientation == Orientation.WEST
					&& canAttach(target.getRelative(BlockFace.WEST)))
				return 5;
		}

		// Try adjacent:
		if (canAttach(target.getRelative(BlockFace.NORTH)))
			return 3;
		if (canAttach(target.getRelative(BlockFace.EAST)))
			return 4;
		if (canAttach(target.getRelative(BlockFace.SOUTH)))
			return 2;
		if (canAttach(target.getRelative(BlockFace.WEST)))
			return 5;
	...
	/**
	 * Checks if the block can attach a ladder. Not the same block where the
	 * ladder is located.
	 * 
	 * @param block
	 *            block to attach the ladder to
