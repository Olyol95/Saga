...
public class Crush extends Ability {
	...
	public boolean handleInteractPreTrigger(PlayerInteractEvent event) {
		return !getSagaLiving().isFalling() && !getSagaLiving().isGrounded()
				&& !event.getPlayer().isSprinting() && super.handlePreTrigger();
	}
	 ...
	 * 
	 * @see
	 * org.saga.abilities.Ability#handleDefendPreTrigger(org.saga.listeners.
	 * events.SagaEntityDamageEvent)
	 */
	@Override
	 ...
	 * Starts the charge.
	 * 
	 * @see org.saga.abilities.Ability#triggerInteract(org.bukkit.event.player.
	 * PlayerInteractEvent)
	 */
	@Override
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		// Horizontal:
		double horSpeed = getDefinition().getFunction(HORIZONTAL_SPEED_KEY)
				.value(getScore());
		...
		// Vertical:
		double vertSpeed = getDefinition().getFunction(VERTICAL_SPEED_KEY)
				.value(getScore());
		...
		// Down velocity:
		long duration = (long) (getDefinition().getFunction(DURATION_KEY)
				.value(getScore()) * 20);
		Saga.plugin().getServer().getScheduler()
				.scheduleSyncDelayedTask(Saga.plugin(), new Runnable() {
					...
					public void run() {
						...
						double speed = getDefinition().getFunction(
								DOWN_SPEED_KEY).value(getScore());
	...
	public boolean triggerDefend(SagaDamageEvent event) {
		...
		if (event.sagaDefender instanceof SagaPlayer) {
			...
			Player defenderPlayer = ((SagaPlayer) event.sagaDefender)
					.getPlayer();

			// Blocking:
			if (!defenderPlayer.isBlocking())
				return false;
		...
		double pushRadius = getDefinition().getFunction(PUSH_RADIUS_KEY).value(
				getScore());
		double pushSpeed = getDefinition().getFunction(PUSH_SPEED_KEY).value(
				getScore());

		List<Entity> nearby = attacker.getNearbyEntities(pushRadius,
		...
		for (Entity entity : nearby) {
			...
			double damage = getDefinition().getFunction(PUSH_DAMAGE_KEY).value(
					getScore());

			// Damage event:
			SagaDamageEvent devent = new SagaDamageEvent(new EntityDamageEvent(
					attacker, DamageCause.FALLING_BLOCK, damage));
		...
		// Damage threshold:
		if (event.getBaseDamage() <= getDefinition().getFunction(
				DAMAGE_THRESHOLD_KEY).intValue(getScore())) {
			trigger = true;
			event.cancel();
		}
