...
public class Repair extends Ability {
	...
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		Integer enchLevelsMax = getDefinition().getFunction(
				ENCHANTMENT_COST_KEY).intValueCeil(enchScore);
		Integer enchLevels = getDefinition().getFunction(ENCHANTMENT_COST_KEY)
				.randomIntValue(enchScore);
		Double enchMult = getDefinition().getFunction(ENCHANTMENT_REPAIR_KEY)
				.value(enchScore);

		// Repair amount:
		short repair = (short) (item.getType().getMaxDurability() * getRepairPercent(item
				.getType()));
		if (enchScore > 0)
			repair *= enchMult;
		...
		if (item.getDurability() <= 0) {
			sagaPlayer.message(AbilityMessages.repairAlreadyRepaired(item
					.getType()));
			return false;
		}
		...
		if (repair == 0 || usedMaterial == Material.AIR) {
			sagaPlayer
					.message(AbilityMessages.repairCantRepair(item.getType()));
			return false;
		}
		...
		// sagaPlayer.message(AbilityMessages.insufficientItems(this,
		// usedMaterial, 1));
		// return false;
		// }
		...
		if (enchScore > 0) {
			...
			if (player.getLevel() < enchLevelsMax) {
				sagaPlayer.message(AbilityMessages
						.repairLevelsRequired(enchLevelsMax));
				return false;
			}
		...
		// Use enchant levels:
		if (enchScore > 0)
			player.setLevel(player.getLevel() - enchLevels);

		// Use items:
		Boolean consume = getDefinition().getFunction(CONSUME_CHANCE_KEY)
				.randomBooleanValue(skillLevel);
		if (consume)
		...
		sagaPlayer.playGlobalEffect(Effect.STEP_SOUND,
				Material.IRON_BLOCK.getId(), location);
