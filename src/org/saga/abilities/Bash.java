...
public class Bash extends Ability {
	...
	public boolean triggerAttack(SagaDamageEvent event) {
		...
		if (event.type != DamageType.MELEE && event.type != DamageType.RANGED)
			return false;
		...
		if (event.type == DamageType.MELEE) {
			disarm = getDefinition().getFunction(MELEE_DISARM_CHANCE_KEY)
					.value(getScore());
		} else {
			disarm = getDefinition().getFunction(RANGED_DISARM_CHANCE_KEY)
					.value(getScore());
		}
		if (random.nextDouble() > disarm)
			return false;
		...
		ItemStack secondStack = defenderPlayer.getInventory().getItem(
				secondSlot);
		...
		if (secondStack != null && !secondStack.getType().equals(Material.AIR))
			defenderPlayer.getInventory().setItem(firstSlot, secondStack);
		if (firstStack != null && !firstStack.getType().equals(Material.AIR))
			defenderPlayer.getInventory().setItem(secondSlot, firstStack);
