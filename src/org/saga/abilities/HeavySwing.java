...
public class HeavySwing extends Ability {
	...
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		// Skill functions:
		Integer size = getDefinition().getFunction(SIZE_KEY).randomIntValue(
				abilityLevel);
		Integer crumble = getDefinition().getFunction(CRUMBLE_KEY)
				.randomIntValue(abilityLevel);

		// Get shape:
		RelativeShape shape = SPAHPE;
		ArrayList<Block> blocks = shape.getBlocks(targetLocation,
				getSagaLiving().getOrientation(), size);
		...
		while (crumble > 0 && crumbleBlocks.size() > 0) {
			...
			Block crumbleBlock = crumbleBlocks.remove(random
					.nextInt(crumbleBlocks.size()));
			crumble--;
