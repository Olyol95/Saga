...
public class Dig extends Ability {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.saga.abilities.Ability#instant(org.bukkit.event.player.
	 * PlayerInteractEvent)
	 */
	@Override
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		// Get blocks:
		double radius = getDefinition().getFunction(RADIUS_KEY).intValue(
				abilityLevel);
		...
		for (int modx = -boxRadius; modx <= boxRadius; modx++) {
			for (int mody = -boxRadius; mody <= boxRadius; mody++) {
				for (int modz = -boxRadius; modz <= boxRadius; modz++) {
					...
					Block relative = event.getClickedBlock().getRelative(modx,
							mody, modz);
		...
		// Randomisation:
		Integer randomAmount = getDefinition().getFunction(
				RANDOMNESS_AMOUNT_KEY).randomIntValue(abilityLevel);
	 ...
	 *            blocks to remove
	 * @param randoms
	 *            random block to remove
	 * @param randomness
	 *            amount of random blocks to remove
	 * @return true if block were removd
	 */
	private boolean breakBlock(Player player, ArrayList<Block> blocks,
			ArrayList<Block> randoms, Integer randomness) {
