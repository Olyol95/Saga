...
public class Charge extends Ability {
	...
	public boolean handleInteractPreTrigger(PlayerInteractEvent event) {
		return event.getPlayer().isSprinting() && getSagaLiving().isGrounded()
				&& super.handlePreTrigger();
	}
	...
	private void handleCharging(final SagaLiving sagaLiving) {
		...
		if (time != null && System.currentTimeMillis() - time >= duration) {
			...
			if (attacker instanceof Player)
				((Player) attacker).setSneaking(false);
		...
		if (push) {
			...
			// Push:
			double pushRadius = getDefinition().getFunction(PUSH_RADIUS_KEY)
					.value(getScore());
			double pushSpeed = getDefinition().getFunction(PUSH_SPEED_KEY)
					.value(getScore());
			...
			for (Entity entity : nearby) {
				...
				double damage = getDefinition().getFunction(PUSH_DAMAGE_KEY)
						.value(getScore());

				// Damage event:
				SagaDamageEvent event = new SagaDamageEvent(
						new EntityDamageEvent(attacker,
								DamageCause.FALLING_BLOCK, damage));
