...
public abstract class Ability extends SagaCustomSerialization implements
		SecondTicker {
	...
	// Clock:
	/**
	 * Updates the clock state. Use this method with {@link #checkClock()} if
	 * you need second ticks.
	 * 
	...
	public boolean checkItems() {
		...
		PlayerInventory inventory = ((SagaPlayer) sagaLiving).getPlayer()
				.getInventory();
	...
	public boolean checkHeldItem() {
		...
		if (itemHand == null)
			itemHand = new ItemStack(Material.AIR);

		return getDefinition().getItemRestrictions().contains(
				itemHand.getType())
				|| getDefinition().getItemRestrictions().size() == 0;
	...
	public boolean checkCooldownTicks() {
		return sagaLiving.getTicksLived() - triggeredTicks > definition
				.getCooldownTicks();
	}
	...
	public final boolean handlePreTrigger() {
		...
		if (isCooldown()) {
			...
			if (getCooldown() != lastCooldown
					&& getDefinition().getActivationAction() != ActivationAction.NONE
					&& !useSilentPreTrigger()) {
		...
		if (!checkItems()) {
			...
			if (!useSilentPreTrigger())
				sagaLiving.message(AbilityMessages.insufficientItems(this,
						definition.getUsedItem(),
						definition.getUsedAmount(getScore())));
			return false;
		...
		if (!checkEnergy()) {
			...
			if (!useSilentPreTrigger())
				sagaLiving
						.message(AbilityMessages.insuficientEnergy(getName()));
			return false;
