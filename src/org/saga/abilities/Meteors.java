...
public class Meteors extends Ability {
	...
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		TrapezoidGrid trapezoid = new TrapezoidGrid(1.0, width, height, bottom,
				step);
		...
		ArrayList<ArrayList<Point>> grid = trapezoid.create(shift, loc.getX(),
				loc.getZ(), rot);

		// Number of fireballs:
		int[] fireballs = new int[grid.size()];
		double densMult = getDefinition().getFunction(DENSITY_MULTIPLIER_KEY)
				.value(getScore());
		TwoPointFunction densityFunction = getDefinition().getFunction(
				DENSITY_KEY);
		...
		final double speed = getDefinition().getFunction(SPEED_KEY).value(
				getScore());
		int count = 0;
		...
		while (!shoorOrder.isEmpty()) {
			...
			final Location target = new Location(world, point.getX(),
					world.getHighestBlockYAt(loc) + SHOOT_HEIGHT, point.getZ());

			if (Math.abs(target.getY() - loc.getY()) > SHOOT_HEIGHT)
				continue;
			trigger = true;
	 ...
	 */
	public Fireball shootFireball(LivingEntity shooter, Location shootLocation,
			Double speed) {
