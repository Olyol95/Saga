...
public class ExplodingArrow extends Ability {
	...
	// Double radius =
	// getDefinition().getSecondaryFunction().value(getSkillLevel());
	//
	// Double radiusSqr = radius * radius;
	// SagaPlayer sagaPlayer = getSagaPlayer();
	//
	// Location location = event.getEntity().getLocation();
	// List<Entity> entities = event.getEntity().getNearbyEntities(radius,
	// radius, radius);
	//
	// // Call events:
	...
	//
	// SagaPlayer targetPlayer = Saga.plugin().getSagaPlayer(((Player)
	// entity).getName());
	//
	// if(targetPlayer == sagaPlayer || targetPlayer == null) continue;
	//
	// // Pvp event:
	// EntityDamageByEntityEvent edbeEvent = new
	// EntityDamageByEntityEvent(sagaPlayer.getPlayer(),
	// targetPlayer.getPlayer(), DamageCause.PROJECTILE, 1);
	...
	//
	// // Create explosion:
	// location.getWorld().createExplosion(location, strength.floatValue(),
	// false);
	//
	// // Award exp:
