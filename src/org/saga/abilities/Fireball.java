...
public class Fireball extends Ability {
	...
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		Vector shootDirection = event.getPlayer().getEyeLocation()
				.getDirection().normalize();

		Location shootLocation = event.getPlayer().getEyeLocation()
				.add(shootDirection);

		sagaLiving.shootFireball(
				getDefinition().getFunction(SPEED_KEY).value(getScore()),
				shootLocation);
