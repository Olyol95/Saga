...
public class Parry extends Ability {
	...
	public boolean triggerDefend(SagaDamageEvent event) {
		...
		if (time != null) {
			...
			if (getDefinition().getFunction(ACTIVE_TIME).value(getScore()) * 1000.0 >= System
				...
				if (sagaLiving instanceof SagaPlayer)
					StatsEffectHandler.playAnimateArm((SagaPlayer) sagaLiving);
		...
		// double speed =
		// getDefinition().getFunction(PUSHBACK).value(getCooldown());
		// double distance =
		// Math.abs(attacker.getLocation().distance(defender.getLocation()));
		// if(distance < 1.0) distance = 1.0;
		// speed/= distance;
		// Vector velocity = event.defenderPlayer.pushAwayEntity(attacker,
		// speed);
