...
public class Berserk extends Ability {
	...
	public boolean triggerAttack(SagaDamageEvent event) {
		...
		// Enough hits:
		if (hit < getDefinition().getFunction(HITS_REQUIRED).value(getScore()))
			return false;

		// Increase damage:
		double mult = getDefinition().getFunction(DAMAGE_MULTIPLIER).value(
				getScore());
	 ...
	 * Resets hits.
	 * 
	 * @see org.saga.abilities.Ability#triggerDefend(org.saga.listeners.events.
	 * SagaEntityDamageEvent)
	 */
	@Override
