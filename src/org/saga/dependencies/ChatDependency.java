...
public class ChatDependency {
	...
	public static void enable() {
		...
		try {
			...
			RegisteredServiceProvider<net.milkbowl.vault.chat.Chat> economyProvider = Saga
	...
	public static String format(String format, SagaPlayer sagaPlayer) {
		...
		if (bundle != null) {
			format = format.replace(SETTLEMENT_INSERT, GeneralConfiguration
					.config().settlementFormat.replace(
					GeneralConfiguration.INSERT_STRING, bundle.getName()));
		} else {
		...
		Faction faction = sagaPlayer.getFaction();
		if (faction != null && faction.isFormed()) {
			format = format.replace(FACTION_INSERT, GeneralConfiguration
					.config().factionFormat.replace(
					GeneralConfiguration.INSERT_STRING, faction.getName()));
			format = format.replace(FACTION_COLOUR1_INSERT, faction
					.getColour1().toString());
			format = format.replace(FACTION_COLOUR2_INSERT, faction
					.getColour2().toString());
		} else {
		...
		if (role != null) {
			format = format
					.replace(ROLE_INSERT,
							GeneralConfiguration.config().roleFormat.replace(
									GeneralConfiguration.INSERT_STRING,
									role.getName()));
		} else {
		...
		if (rank != null) {
			format = format
					.replace(RANK_INSERT,
							GeneralConfiguration.config().rankFormat.replace(
									GeneralConfiguration.INSERT_STRING,
									rank.getName()));
		} else {
	...
	public static void updatePrefix(SagaPlayer sagaPlayer, Faction faction) {
		...
		if (GeneralConfiguration.isDisabled(player.getWorld()))
			return;
	...
	private static void setGroupManagerPrefix(Player player, String prefix) {
		...
		final OverloadedWorldHolder handler = manager.groupManager
				.getWorldsHolder().getWorldData(player);
		if (handler == null) {
			return;
		}

		handler.getUser(player.getName()).getVariables()
