...
public class InventoryUtil {
	...
	public static void removeItem(ItemStack itemStack, Inventory inventory) {
		...
		if (remaining != null) {
			SagaLogger.warning(InventoryUtil.class, "failed to remove "
					+ itemStack + " from the inventory");
		}
	 ...
	 *            player
	 */
	public static void removeItem(Material material, Integer toRemove,
			boolean removeEnch, Player player) {
		...
		if (toDelete > 0) {
			SagaLogger.severe("Failed to remove " + toDelete + " " + material
					+ " from players " + player.getName() + " inventory");
		}
	...
	public static SagaItem takeItem(SagaItem requested, Inventory inventory) {
		...
		if (remaining != null)
			taken.modifyAmount(-remaining.getAmount());
	...
	/**
	 * Gives a Saga item to a players inventory. Drops the item if full.
	 * 
	 * @param sagaItem
	 *            saga item
	 * @param inventory
	 *            inventory to put to
	 * @param dropLoc
	 *            drop location if full
	 */
	public static void giveItem(SagaItem sagaItem, Inventory inventory,
			Location dropLoc) {
		...
		if (remaining != null)
			dropLoc.getWorld().dropItem(dropLoc, remaining);
