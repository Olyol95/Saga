...
public class ChatTable {
	...
	private Double getColumnWidth(int i) {
		...
		if (customWidths == null || i >= customWidths.size() || i < 0)
			return ChatFiller.CHAT_WIDTH / cells.size();
		...
		for (int rowind = 0; rowind < rows; rowind++) {
			...
			for (int colind = 0; colind < cells.size(); colind++) {
				...
				if (rowind < column.size()) {
					...
					row += ChatFiller.fillString(column.get(rowind),
							widths[colind]);
		...
		for (int col = 0; col < cols; col++) {
			...
			for (int row = 0; row < rows; row++) {
				...
				if (row < cells.get(col).size()) {
					result[row][col] = ChatFiller.fillString(cells.get(col)
							.get(row), width);
				} else {
		...
		for (int row = 0; row < rows; row++) {
			...
			for (int colInd = 0; colInd < cells.size(); colInd++) {
				...
				if (row < column.size()) {
					...
					result.append(ChatFiller.fillString(column.get(row),
							widths[colInd]));
