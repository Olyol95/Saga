...
public class RelativeShape {
	...
	public ArrayList<Block> getBlocks(Location anchor, Orientation orientation,
		...
		for (int i = 0; i < xoffsets.size(); i++) {
			...
			Block block = anchor.getBlock().getRelative(
					getXOffset(orientation, i), getYOffset(orientation, i),
					getZOffset(orientation, i));

			if (pFilter != null && !pFilter.checkBlock(block))
				continue;

			Integer reqLevel = reqLevels.get(i);
			if (reqLevel != -1 && level < reqLevel)
				continue;
	 ...
	 */
	public final int getXOffset(Orientation orientation, int index)
			throws IndexOutOfBoundsException {
	 ...
	 *             when the index is invalid
	 */
	public final int getYOffset(Orientation orientation, int index)
			throws IndexOutOfBoundsException {
	 ...
	 *             when the index is invalid
	 */
	public final int getZOffset(Orientation orientation, int index)
			throws IndexOutOfBoundsException {
