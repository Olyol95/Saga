...
public class TradingPost extends ProductionBuilding implements DaytimeTicker {
	...
	public TradingPost(BuildingDefinition definition) {
		...
		importCoins = 0.0;
		exportCoins = 0.0;
		collectedExports = new double[EconomyConfiguration.config()
				.getTradingPostExports().length];
		exportsWork = new double[EconomyConfiguration.config()
				.getTradingPostExports().length];
	...
	public boolean complete() throws InvalidBuildingException {
		...
		if (collectedExports.length != EconomyConfiguration.config()
				.getTradingPostExports().length) {
		...
		if (exportsWork.length != EconomyConfiguration.config()
				.getTradingPostExports().length) {
	...
	public void perform() {
		...
		// Signs collect:
		ArrayList<Warehouse> warehouses = getChunkBundle().getBuildings(
				Warehouse.class);
	...
	protected boolean isBuildingSign(String firstLine) {
		...
		if (firstLine.equalsIgnoreCase(BuySign.SIGN_NAME))
			return true;
		if (firstLine.equalsIgnoreCase(SellSign.SIGN_NAME))
			return true;
		if (firstLine.equalsIgnoreCase(ImportSign.SIGN_NAME))
			return true;
		if (firstLine.equalsIgnoreCase(ExportSign.SIGN_NAME))
			return true;
	 ...
	 * 
	 * @see
	 * org.saga.buildings.Building#createBuildingSign(org.bukkit.block.Sign,
	...
	protected BuildingSign createBuildingSign(Sign sign, SignChangeEvent event) {
		...
		if (event.getLine(0).equalsIgnoreCase(BuySign.SIGN_NAME))
			return BuySign.create(sign, event.getLine(1), event.getLine(2),
					event.getLine(3), this);
		if (event.getLine(0).equalsIgnoreCase(SellSign.SIGN_NAME))
			return SellSign.create(sign, event.getLine(1), event.getLine(2),
					event.getLine(3), this);
		if (event.getLine(0).equalsIgnoreCase(ImportSign.SIGN_NAME))
			return ImportSign.create(sign, event.getLine(1), event.getLine(2),
					event.getLine(3), this);
		if (event.getLine(0).equalsIgnoreCase(ExportSign.SIGN_NAME))
			return ExportSign.create(sign, event.getLine(1), event.getLine(2),
					event.getLine(3), this);
	 ...
	 * 
	 * @see
	 * org.saga.buildings.Building#getBuildingSignCreatePermission(java.lang
	 * .String)
	 */
	@Override
	protected SettlementPermission getBuildingSignCreatePermission(
		...
		if (firstLine.equalsIgnoreCase(BuySign.SIGN_NAME))
			return SettlementPermission.MANAGE_BUY;
		if (firstLine.equalsIgnoreCase(SellSign.SIGN_NAME))
			return SettlementPermission.MANAGE_SELL;
		if (firstLine.equalsIgnoreCase(ImportSign.SIGN_NAME))
			return SettlementPermission.MANAGE_IMPORT;
		if (firstLine.equalsIgnoreCase(ExportSign.SIGN_NAME))
			return SettlementPermission.MANAGE_EXPORT;
	 ...
	 * 
	 * @see
	 * org.saga.buildings.Building#getBuildingSignRemovePermission(java.lang
	 * .String)
	 */
	@Override
	...
	public void work() {
		...
		for (int i = 0; i < workRemain.length; i++) {
			...
			workRemain[i] = exports[i].getRequiredWork() * percent
					- exportsWork[i];
			if (workRemain[i] < 0.0)
				workRemain[i] = 0.0;
		...
		for (String roleName : roles) {
			...
			if (requested >= 0)
				workAvail += settlement.takeWorkPoints(roleName, requested);
	...
	public void produce() {
		...
		for (int i = 0; i < exports.length; i++) {
			...
			if (exportsWork[i] < exports[i].getRequiredWork())
				continue;
		...
		if (getChunkBundle() instanceof Settlement) {
			...
			if (coins >= 0.0)
				settlement.payCoins(coins);

			// Inform:
			if (exported.size() != 0)
				settlement.information(this,
						EconomyMessages.exported(exported, coins));
	 ...
	 * 
	 * @see
	 * org.saga.buildings.production.ProductionBuilding#offer(org.saga.buildings
	 * .production.SagaItem)
	 */
	@Override
	public void offer(SagaItem item) {
		...
		for (int i = 0; i < exports.length; i++) {
			...
			if (!exports[i].checkRepresents(item))
				continue;

			double amount = item.getAmount();
			if (collectedExports[i] + amount > exports[i].getAmount())
				amount = exports[i].getAmount() - collectedExports[i];
