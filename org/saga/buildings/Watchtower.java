...
public class Watchtower extends Building implements DaytimeTicker {
	...
	public void enable() {
		...
		ArrayList<SagaChunk> protectedChunks = sagaChunk
				.getAdjacent(getRadius());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.saga.buildings.Building#onCreatureSpawn(org.bukkit.event.entity.
	 * CreatureSpawnEvent, org.saga.chunkGroups.SagaChunk)
	 */
	@Override
	public void onCreatureSpawn(CreatureSpawnEvent event,
		...
		if (event.getLocation().getY() < event.getLocation().getWorld()
				.getSeaLevel())
			return;
		...
		String spottedName = event.getEntity().getClass().getSimpleName()
				.replace("Craft", "").toLowerCase();
		Integer spottedAmount = mobReport.get(spottedName);
		if (spottedAmount == null)
			spottedAmount = 0;
		spottedAmount++;
	...
	public ArrayList<String> getSpecificStats() {
		...
		rList.add("Spot radius: " + getRadius() + " Protected: "
