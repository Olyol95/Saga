...
public class AbilitySign extends BuildingSign {
	 ...
	 *            building
	 */
	private AbilitySign(Sign sign, String secondLine, String thirdLine,
	 ...
	 */
	public static AbilitySign create(Sign sign, String secondLine,
			String thirdLine, String fourthLine, Building building) {
		return new AbilitySign(sign, secondLine, thirdLine, fourthLine,
				building);
	}
	...
	protected void onRightClick(SagaPlayer sagaPlayer) {
		...
		if (sagaPlayer.getRemainingAbilityPoints() < 1) {
			sagaPlayer
					.message(BuildingMessages.abilityPointsRequired(abilName));
			return;
		}

		// Requirements:
		if (!definition.checkRequirements(sagaPlayer, nextScore)) {
			sagaPlayer.message(BuildingMessages.abilityRequirementsNotMet(
					abilName, nextScore));
			return;
		}
		...
		// // Enough coins:
		// Double cost =
		// EconomyConfiguration.config().getAbilityUpgradeCost(abilityName,
		// abilityScore);
		// if(sagaPlayer.getCoins() < cost){
		// sagaPlayer.message(EconomyMessages.notEnoughCoins(cost,
		// sagaPlayer.getCoins()));
		// return;
		// }
		//
		...
		if (ability == null) {
			SagaLogger.severe(this, "failed to force " + abilName
					+ " ability from " + sagaPlayer.getName());
