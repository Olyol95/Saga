...
public class Shearing extends Ability {
	 ...
	 * 
	 * @see
	 * org.saga.abilities.Ability#handleShearPreTrigger(org.bukkit.event.player
	...
	public boolean triggerShear(PlayerShearEntityEvent event) {
		...
		if (event.getEntity() instanceof Sheep) {
			...
			Integer bonus = getDefinition().getFunction(BONUS_WOOL_KEY)
					.randomIntValue(getScore());
			if (bonus < 1)
				return false;
		...
		// Mushroom cow:
			...
			Integer bonus = getDefinition().getFunction(BONUS_MUSHROOMS_KEY)
					.randomIntValue(getScore());
			if (bonus < 1)
				return false;
