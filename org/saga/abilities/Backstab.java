...
public class Backstab extends Ability {
	 ...
	 * 
	 * @see
	 * org.saga.abilities.Ability#handleAttackPreTrigger(org.saga.listeners.
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.saga.abilities.Ability#triggerAttack(org.saga.listeners.events.
	 * SagaEntityDamageEvent)
	 */
	public boolean triggerAttack(SagaDamageEvent event) {
		...
		// Check degrees:
		if (Math.abs(deg) > getDefinition().getFunction(FACING_HALF_ANGLE)
				.value(getCooldown()))
			return false;

		// Backstab:
		event.multiplyDamage(getDefinition().getFunction(DAMAGE_MULTIPLIER)
				.value(getScore()));
		event.modifyArmourPenetration(getDefinition().getFunction(
				ARMOUR_PENETRATION).value(getScore()));
		event.modifyEnchantPenetration(getDefinition().getFunction(
				ENCHANT_PENETRATION).value(getScore()));
	...
	/**
	 * Gets the degrees between a line connecting both entities and the
