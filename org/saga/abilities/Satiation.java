...
public class Satiation extends Ability {
	 ...
	 * 
	 * @see
	 * org.saga.abilities.Ability#triggerFoodLevelChange(org.bukkit.event.entity
	...
	public boolean triggerFoodLevelChange(FoodLevelChangeEvent event) {
		...
		if (event.getEntity() instanceof Player) {
			...
			if (change > 0) {
				...
				change = TwoPointFunction.randomRound(change
						* getDefinition().getFunction(GAIN_MULTIPLIER_KEY)
								.value(getScore()));
				...
				change = TwoPointFunction.randomRound(change
						* getDefinition().getFunction(LOOSE_MULTIPLIER_KEY)
								.value(getScore()));
				event.setFoodLevel(current + change);
