...
public class Counterattack extends Ability {
	 ...
	 * 
	 * @see
	 * org.saga.abilities.Ability#onPlayerInteractPlayer(org.bukkit.event.player
	 * .PlayerInteractEntityEvent, org.saga.player.SagaPlayer)
	 */
	@Override
	public boolean triggerDefend(SagaDamageEvent event) {
		...
		// Blocking:
		if (!sagaDefender.getPlayer().isBlocking())
			return false;

		// Attacker:
		LivingEntity attacker = null;
		if (event.sagaAttacker != null)
			attacker = event.sagaAttacker.getWrapped();
		else if (event.creatureAttacker != null)
			attacker = event.creatureAttacker;
		else
			return false;

		// Check degrees:
		double deg = Parry.getFacing(defender, attacker);
		if (Math.abs(deg - 180) > getDefinition()
				.getFunction(FACING_HALF_ANGLE).value(getCooldown()))
			return false;
		...
		// Call event:
		progress = true;
		EntityDamageByEntityEvent edbeEvent = new EntityDamageByEntityEvent(
				defender, attacker, DamageCause.ENTITY_ATTACK,
				VanillaConfiguration.getBaseDamage(weapon));
		...
		if (event.sagaAttacker instanceof SagaPlayer) {
			...
			damage *= VanillaConfiguration.getArmourMultiplier(edbeEvent,
					event.sagaAttacker.getWrapped());
			damage *= VanillaConfiguration.getEPFMultiplier(edbeEvent,
					event.sagaAttacker.getWrapped());
			if (VanillaConfiguration.checkBlocking(edbeEvent,
					event.sagaAttacker.getWrapped()))
