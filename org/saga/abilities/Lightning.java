...
public class Lightning extends Ability {
	...
	public boolean triggerInteract(PlayerInteractEvent event) {
		...
		// Target:
		Block block = sagaLiving.getWrapped().getTargetBlock(null,
		...
		LightningStrike lightning = target.getWorld().strikeLightningEffect(
				target);
		...
		for (Entity entity : nearby) {
			...
			if (entity instanceof LivingEntity
					&& entity.getLocation().distanceSquared(target) <= range2) {
		...
		for (LivingEntity living : filteredNearby) {
			...
			EntityDamageByEntityEvent bevent = new EntityDamageByEntityEvent(
					shooter, living, DamageCause.LIGHTNING, damage);
			...
			// Play effect:
			living.getLocation().getWorld()
					.playEffect(living.getLocation(), Effect.SMOKE, 4);
