...
public class BlockListener implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		// Saga disabled:
		if (GeneralConfiguration.isDisabled(event.getPlayer().getLocation()
				.getWorld()))
			return;
		...
		// Saga loot event:
		SagaLootEvent lootEvent = new SagaLootEvent(event, sagaPlayer,
				sagaChunk);
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		if (!block.hasMetadata(UnnaturalTag.METADATA_KEY)) {
			block.setMetadata(UnnaturalTag.METADATA_KEY,
					UnnaturalTag.METADATA_VALUE);
		}
	...
	public void onBlockSpread(BlockSpreadEvent event) {
		...
		// Saga disabled:
		if (GeneralConfiguration.isDisabled(event.getBlock().getWorld()))
			return;

		if (!canSpread(event.getSource().getType(), event.getSource()
				.getLocation(), event.getBlock().getLocation()))
			event.setCancelled(true);
	...
	public void onBlockFromTo(BlockFromToEvent event) {
		...
		if (!canSpread(event.getBlock().getType(), event.getBlock()
				.getLocation(), event.getToBlock().getLocation()))
			event.setCancelled(true);
	...
	private static boolean canSpread(Material mat, Location from, Location to) {
		...
		if (fromChunk != null)
			fromBundle = fromChunk.getBundle();
		if (toChunk != null)
			toBundle = toChunk.getBundle();

		if (toBundle != null) {

			// Inward spread:
			if (SettlementConfiguration.config().getInwardSpreadProtection()
					&& fromBundle != toBundle)
				return false;

			// Fire spread:
			if (mat == Material.FIRE && toBundle == fromBundle)
				return false;
