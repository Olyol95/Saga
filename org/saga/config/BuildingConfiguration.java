...
public class BuildingConfiguration {
	 ...
	 *             if building definition is missing
	 */
	public Building createBuilding(String name) throws InvalidBuildingException {
		...
		try {
			...
			Constructor<? extends Building> co = clca
					.getConstructor(BuildingDefinition.class);
			return co.newInstance(definition);

		} catch (Throwable e) {

			throw new InvalidBuildingException(name, e.getClass()
	...
	public static BuildingConfiguration load() {
		...
		if (!WriterReader.checkExists(Directory.BUILDING_CONFIG)) {
			...
			try {
				...
				SagaLogger.severe(BuildingConfiguration.class,
		...
		try {
			...
			config = WriterReader.readConfig(Directory.BUILDING_CONFIG,
