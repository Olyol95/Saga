...
public class AbilityConfiguration {
	...
	public void complete() {
		...
		for (int i = 0; i < definitions.size(); i++) {
			...
			try {
				...
				SagaLogger.severe(getClass(), "failed to complete "
						+ definitions.get(i).getName() + " definition: "
	...
	public static Ability createAbility(String name)
		...
		try {
			...
			Constructor<? extends Ability> co = clca
					.getConstructor(AbilityDefinition.class);
			return co.newInstance(definition);

		} catch (Throwable e) {

			throw new InvalidAbilityException(name, e.getClass()
	...
	public static AbilityConfiguration load() {
		...
		if (!WriterReader.checkExists(Directory.ABILITY_CONFIG)) {
			...
			try {
				...
				SagaLogger.severe(AbilityConfiguration.class,
		...
		try {
			...
			config = WriterReader.readConfig(Directory.ABILITY_CONFIG,
					AbilityConfiguration.class);
