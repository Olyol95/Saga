...
public class ExperienceConfiguration {
	...
	public Double getExp(Creature creature) {
		...
		Double exp = creatureExp.get(creature.getClass().getSimpleName()
				.toLowerCase().replace("craft", ""));

		if (exp == null)
			exp = creatureExp.get("default");
	...
	public static ExperienceConfiguration load() {
		...
		if (!WriterReader.checkExists(Directory.EXPERIENCE_CONFIG)) {
			...
			try {
				...
				SagaLogger.severe(ExperienceConfiguration.class,
		...
		try {
			...
			config = WriterReader.readConfig(Directory.EXPERIENCE_CONFIG,
