...
public class WarMessages {
	...
	public static String notOwned(Faction faction, Bundle bundle) {
		return Colour.negative + "The faction doesn't own " + bundle.getName()
				+ " settlement.";
	}

	public static String siegeSpawnDeny(Faction faction) {
		return Colour.negative
				+ "Can't spawn in settlements that are under siege.";
	}
	...
	public static String siegeAttackReminder(Faction faction, Bundle bundle) {
		...
		if (minutes > 0) {
			...
			return faction.getColour2() + "Siege on " + bundle.getName()
					+ " settlement starts in "
					+ GeneralMessages.durationDHM(durationHM) + ".";

		} else {

			return faction.getColour2() + "Attack " + bundle.getName()
					+ " settlement!";
	...
	public static String siegeDefendReminder(Faction faction, Bundle bundle) {
		...
		if (minutes > 0) {
			...
			Duration durationHM = new Duration(minutes * 60000);
			return faction.getColour2() + "Defence of " + bundle.getName()
					+ " settlement starts in "
					+ GeneralMessages.durationDHM(durationHM) + ".";

		} else {

			return faction.getColour2() + "Defend " + bundle.getName()
					+ " settlement!";
	...
	public static String warAlreadyDeclared(Faction faction, Faction target) {
		return Colour.negative + "War already declared on "
				+ FactionMessages.faction(target) + " faction.";
	}

	public static String warDeclaredOn(Faction faction, Faction target) {
		return faction.getColour2() + "War declared on "
				+ FactionMessages.faction(target) + " faction.";
	}

	public static String warDeclaredBy(Faction faction, Faction target) {
		return faction.getColour2() + "Faction "
				+ FactionMessages.faction(target) + " declared war.";
	}

	public static String isAtWarDeny(Faction faction, Faction target) {
		return Colour.negative + "The faction is at war with "
				+ FactionMessages.faction(target) + ".";
	}
	...
	public static String warDeclareWait(Faction faction, Faction target,
			Duration duration) {
		return Colour.negative + "Need to wait "
				+ GeneralMessages.durationDHM(duration)
				+ " before a war can be declared on "
				+ FactionMessages.faction(target) + " faction.";
	}

	// Peace:
	public static String peaceDeclaredOn(Faction faction, Faction target) {
		return faction.getColour2() + "Peace declared with "
				+ FactionMessages.faction(target) + " faction.";
	}

	public static String peaceDeclaredBy(Faction faction, Faction target) {
		return faction.getColour2() + "Faction "
				+ FactionMessages.faction(target) + " declared peace.";
	}

	public static String peaceDeny(Faction faction, Faction target) {
		return Colour.negative + "The faction is not at war with "
				+ FactionMessages.faction(target) + ".";
	}
	...
	public static String limitedMemberCantQuitInfo() {
		return Colour.normal1
				+ "To leave the faction either quit the settlement or join another faction.";
	}
	...
	public static String affiliationSet(Bundle bundle, Faction faction) {
		...
		return Colour.normal2 + "The settlement is now affiliated with "
				+ FactionMessages.faction(faction) + " faction.";

	}

	public static String affiliationRemoved(Bundle bundle) {

		return Colour.normal2
				+ "The settlement is no longer affiliated with a faction.";

	}

	public static String affiliationAlreadySet(Bundle bundle, Faction faction) {

		return Colour.negative + "The settlement is already affiliated with "
				+ FactionMessages.faction(faction) + " faction.";
	...
	public static String capitalNoTownSquare(Faction faction) {
		return Colour.negative + "The capital settlement has no "
				+ HelpMessages.townSquare() + ".";
	}
