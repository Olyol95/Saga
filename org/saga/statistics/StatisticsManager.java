...
public class StatisticsManager implements HourTicker {
	...
	public boolean complete() {
		...
		if (playerAttributes != null) {
			...
			for (String attribute : attributes) {
				...
				Hashtable<String, Integer> players = playerAttributes
						.get(attribute);
				...
				for (String playerName : playerNames) {
					...
					doubleValues.put("attributes" + "." + attribute + "."
							+ playerName, score);
		...
		if (foundVeins != null) {
			...
			for (String player : players) {
				...
				for (Material material : materials) {
					...
					setValue("found_veins" + "." + material.toString() + "."
							+ player, veins);
	...
	public boolean clockHourTick() {
		...
		Integer ageDays = new Double(calcStatisticsAge()
				/ (60.0 * 60.0 * 1000.0)).intValue();
		if (ageDays < GeneralConfiguration.config().statisticsUpdateAge)
			return true;
	...
	// Economy:
	public void onPlayerBuy(SagaPlayer sagaPlayer, Material material,
			Integer amount, Double cost) {
	...
	public void onPlayerSell(SagaPlayer sagaPlayer, Material material,
			Integer amount, Double cost) {
	...
	public Double getBuyCoins(Material material) {
		...
		Collection<Hashtable<Material, Double>> materials = playerBuyCoins
				.values();
	...
	public Integer getBuyAmount(Material material) {
		...
		Collection<Hashtable<Material, Integer>> materials = playerBuyAmount
				.values();
	...
	public Double getSellCoins(Material material) {
		...
		Collection<Hashtable<Material, Double>> materials = playerSellCoins
				.values();
	...
	public Integer getSellAmount(Material material) {
		...
		Collection<Hashtable<Material, Integer>> materials = playerSellAmount
				.values();
	...
	public ArrayList<Material> getAllEcoMaterials() {
		...
		// Sell materials:
		Collection<Hashtable<Material, Double>> sellMaterials = playerSellCoins
				.values();
	...
	public Integer getFoundVeins(String name, Material material) {
		...
		return (int) getValue("found_veins" + "." + material.toString() + "."
				+ name);
	...
	public void confirmXray(String name, Material material, Double ratio) {
		...
		setValue("xray_confirmed" + "." + material.toString() + "." + name,
				ratio);
	...
	public boolean isXrayConfirmed(String name, Material material) {
		...
		return getValue("xray_confirmed" + "." + material.toString() + "."
				+ name) > 0.0;
	...
	public void clearCateg(String category) {
		...
		for (String fullCateg : allCategs) {
			...
			if (!fullCateg.startsWith(category + "."))
				continue;
	...
	public static String formatCategName(String category) {
		...
		return ChatUtil.repeat(GeneralMessages.TAB, calcCategDepth(category))
				+ subCategs[subCategs.length - 1];
	...
	public TreeSet<String> getSubCategs(String category, boolean ignoreBottom) {
		...
		for (String fullCateg : allCategs) {
			...
			if (ignoreBottom) {
				...
				if (lastIndex != -1)
					subCateg = subCateg.substring(0, lastIndex);
			}
			...
			if (subSuperCategs.length > 1) {
				...
				for (int i = 1; i < subSuperCategs.length; i++) {
					...
					if (!subCategs.contains(subSuperCateg))
						subCategs.add(subSuperCateg);
	...
	public Double[] createHistogramData(String category) {
		...
		for (String subCateg : subCategs) {
			...
			data.add(StatisticsManager.manager().getValue(
					category + "." + subCateg));
	...
	public void setPlayer(SagaPlayer sagaPlayer) {
		...
		for (String attribute : attributes) {
			...
			setValue(
					"attributes.trained" + "." + attribute + "."
							+ sagaPlayer.getName(), score);
	...
	public void addGuardRuneRestore(SagaPlayer sagaPlayer) {
		...
		modifyValue("guardrune" + "." + "restore" + "." + sagaPlayer.getName(),
				1);
	...
	public void addGuardRuneRestoreException(SagaPlayer sagaPlayer) {
		...
		modifyValue(
				"guardrune" + "." + "restore_exception" + "."
						+ sagaPlayer.getName(), 1);
	...
	public void addGuardRuneRecharge(SagaPlayer sagaPlayer) {
		...
		modifyValue(
				"guardrune" + "." + "recharge" + "." + sagaPlayer.getName(), 1);
	...
	public void addAbilityUse(Ability ability) {
		...
		String abilityName = ability.getName() + "." + ability.getName() + " "
				+ RomanNumeral.binaryToRoman(ability.getScore());

		modifyValue("abilities" + "." + "used" + "." + abilityName + "."
				+ sagaPlayer.getName(), 1);
	...
	public void setBuildings(Bundle bundle) {
		...
		Collection<String> bldgNames = BuildingConfiguration.config()
				.getBuildingNames();

		for (String bldgName : bldgNames) {

			String key = "buildings" + "." + "set" + "." + bldgName + "."
					+ bundle.getId();
			clearValue(key);
	...
	public void addBundleClaimed(Faction faction, Bundle bundle) {
		...
		modifyValue(
				"faction_claiming" + "." + "claimed" + "." + faction.getName(),
				1);
	...
	public void addBundleSeized(Faction faction, Bundle bundle) {
		...
		modifyValue(
				"faction_claiming" + "." + "seized" + "." + faction.getName(),
				1);
	...
	public void addBundleLost(Faction faction, Bundle bundle) {
		...
		modifyValue(
				"faction_claiming" + "." + "lost" + "." + faction.getName(), 1);

	}

	public void setBundlesOwned(Faction faction) {

		setValue("faction_claiming" + "." + "owned" + "." + faction.getName(),
				SiegeManager.manager().getOwnedBundleCount(faction.getId()));

	}

	public void setClaims(Settlement settlement) {

		setValue("settlements.claims" + "." + settlement.getName(),
				settlement.getClaims());

	}

	public void setSize(Settlement settlement) {

		setValue("settlements.size" + "." + settlement.getName(),
				settlement.getSize());
	...
	public void setWallet(SagaPlayer sagaPlayer) {
		...
		setValue("wallet" + "." + sagaPlayer.getName(),
				EconomyDependency.getCoins(sagaPlayer));

	}

	public void setRoles(Settlement settlement) {

		ArrayList<ProficiencyDefinition> proficiencies = ProficiencyConfiguration
		...
		for (ProficiencyDefinition proficiency : proficiencies) {
			...
			setValue("settlements.roles.available" + "." + name + "."
					+ settlement.getName(), available);
			setValue(
					"settlements.roles.used" + "." + name + "."
							+ settlement.getName(), used);
	...
	public void setRanks(Faction faction) {
		...
		for (ProficiencyDefinition proficiency : proficiencies) {
			...
			setValue(
					"factions.ranks.available" + "." + name + "."
							+ faction.getName(), available);
			setValue(
					"factions.ranks.used" + "." + name + "."
							+ faction.getName(), used);

		}

	}

	public void addManminutes(Faction faction, Integer online) {
	...
	public void addBuy(SagaPlayer sagaPlayer, Material material,
			Integer amount, Double price) {
		modifyValue("eco.buy." + material + "." + sagaPlayer.getName(), amount
				* price);
	}

	public void addSell(SagaPlayer sagaPlayer, Material material,
			Integer amount, Double price) {
		modifyValue("eco.sell." + material + "." + sagaPlayer.getName(), amount
				* price);
	}

	public void addImport(SagaPlayer sagaPlayer, Material material,
			Integer amount, Double price) {
		modifyValue("eco.import." + material + "." + sagaPlayer.getName(),
				amount * price);
	}

	public void addExport(SagaPlayer sagaPlayer, Material material,
			Integer amount, Double price) {
		modifyValue("eco.export." + material + "." + sagaPlayer.getName(),
				amount * price);
	}

	public void addBlockCoins(SagaPlayer sagaPlayer, Material material,
			Double coins) {
		modifyValue("eco.block_coins." + material + "." + sagaPlayer.getName(),
				coins);
	}
	...
	public static StatisticsManager load() {
		...
		// Load:
			...
			try {
				...
				instance = WriterReader.read(Directory.STATISTICS, "last",
	...
	public void archive() {
		...
		try {
			...
			String name = dateFormat.format(GregorianCalendar.getInstance()
					.getTime());
			...
			SagaLogger.severe(SettlementConfiguration.class,
