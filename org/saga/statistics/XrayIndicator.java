...
public class XrayIndicator {
	...
	public static void onBlockBreak(SagaPlayer sagaPlayer, BlockBreakEvent event) {
		...
		if (event.getBlock() == null
				|| (event.getBlock().getType() != Material.STONE
						&& event.getBlock().getType() != Material.DIRT && event
						.getBlock().getType() != Material.GRAVEL))
			return;

		// Add stone as a vein:
		if (event.getBlock().getType() == Material.STONE)
			StatisticsManager.manager().addFoundVein(sagaPlayer.getName(),
					Material.STONE);
	 ...
	 *            depth
	 */
	private static void getRelative(Block anchor, HashSet<Block> targetBlocks,
			HashSet<Block> otherBlocks, int depthremain) {

		if (targetBlocks.contains(anchor) || otherBlocks.contains(anchor))
			return;
		if (depthremain < 1)
			return;
